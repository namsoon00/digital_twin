...
같은 화면의 `필요 API 맵`에는 웹 클라이언트에서 직접 호출 가능한 CoinGecko, DefiLlama 연결 상태를 우선 노출합니다. 토스증권 Open API는 브라우저 직접 호출 대상이 아니며, `docs/toss-api-contract.md`의 BFF 계약을 기준으로 다음 단계에서 연결합니다.
`코인 마켓` 카드에서는 CoinGecko API로 BTC, ETH, SOL, XRP, BNB, USDC, USDT, DOGE의 가격, 시총, 거래량, 1h/24h/7d 변화율을 확인할 수 있습니다.
`종합 플로우 캔들`은 캔들형 종합 흐름 지수, 유동성 막대, AI/코인/금/KOSPI/리스크 라인을 하나의 그래프에 겹쳐 보여주며 일별/주별/월별 보기, 1M/2M/3M/ALL 기간, 세부 구간을 조정할 수 있습니다. 그래프와 흐름 카드에는 실제 API 데이터인지 mock 데이터인지 배지로 표시합니다.
...
앱의 `설정` 탭에서는 웹 직접 호출 가능한 데이터 API만 연결 테스트 대상으로 다룹니다. 현재 우선 대상은 CoinGecko와 DefiLlama이며, 키가 필요 없는 공개 호출 또는 선택 API key만 사용합니다.

토스증권 Open API는 OAuth client secret과 계좌 헤더를 다루므로 Flutter Web에서 직접 호출하지 않습니다. 현재 방향은 개인용 네이티브 앱 우선이며, 사용자가 직접 입력한 `client_id`/`client_secret`은 iOS Keychain/Android Keystore 계층에 저장합니다. 1단계 계약과 mock fixture는 `docs/toss-api-contract.md`와 `docs/fixtures/toss/`에 정리되어 있습니다.
...
- 기업 뉴스: CNBC, Yahoo Finance, Fed, 연합뉴스, CoinDesk 등 공개 피드와 로컬 기본 피드
...
저장한 종목은 주식 탭에서 보유/관심 필터와 검색으로 찾고, 카드별로 기업명, 티커, 구분, 수량, 평균단가, 목표가, 관찰 메모를 수정하거나 보유/관심 상태를 전환할 수 있습니다.
