...
`notify`는 한 번 실행하고, `notify:watch`는 기본 10분(`NOTIFY_INTERVAL_MINUTES`) 간격으로 반복 실행합니다. 알림은 토스 연결 실패, 장전/마감 요약, 각 보유 종목의 상태와 매도/추가매수 검토 타이밍, 섹터 집중, 현금 비중 부족을 점검합니다. 한국장과 미국장의 장전/장중/장후/장외 상태를 종목별로 반영하고, 장중이 아닐 때는 개장 후 확인 또는 다음 정규장 재점검 중심으로 안내합니다. 매수/매도 문구는 주문 지시가 아니라 사용자가 확인할 조건과 기준입니다.
