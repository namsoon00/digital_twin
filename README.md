...
토스증권 Open API, 뉴스 피드, X 같은 소셜 포스팅을 함께 읽어 보유/관심 종목의 매도 타이밍, 시장 심리, 종목 흐름을 판단하는 로컬 우선 대시보드입니다. 브라우저는 `/api/flow-lens` 집계 결과만 받고, 토스/X credentials는 서버 환경변수에서만 사용합니다.
...
웹은 `판단`, `심리·흐름`, `피드`, `설정` 하단 탭으로 구성됩니다. `설정` 탭에서는 관심 종목, Toss API 설정, X 검색 설정, secret 값을 입력하고 이 브라우저의 `localStorage`에 저장할 수 있습니다. GitHub Pages에서는 secret을 서버로 보내지 않으며, 브라우저 저장소가 막힌 환경에서는 현재 탭 메모리에서만 유지됩니다.

## 앱 구조

- `public/`: Exit Lens 웹 대시보드
- `GET /api/flow-lens`: 토스 계좌/보유자산, 관심 종목, 뉴스/포스팅 테마, 시장 심리, 종목별 흐름, 매도 압력, 점검 체크리스트 집계
...
시장 심리는 뉴스와 X 포스팅에서 낙관, 공포, 추격, 의견 충돌 신호를 점수화합니다. 종목 흐름은 보유/관심 종목별 관련 신호, 섹터 테마, 매도 압력, 시장 심리를 조합해 `유입 우세`, `혼조`, `약한 이탈`, `이탈 우세`로 읽습니다. 실제 체결 수급이나 호가 데이터가 아니라 텍스트 신호 기반 판독입니다.
