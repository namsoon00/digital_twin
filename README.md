...
웹은 `운영`, `판단`, `실험실`, `모델`, `알림`, `보유`, `피드`, `관심` 하단 탭으로 구성됩니다. 기본 첫 화면인 `운영` 탭에서는 Python 서비스 계정을 등록하고, 메시지 타입별 알림 활성화와 주기, 알림 채널, 모델 공식과 판단 기준을 한 곳에서 관리합니다. `실험실`에서는 매수/매도 실험 기록을 버전으로 저장하고, `모델`에서는 나만의 매수·매도 공식, 가중치, 판단 기준, 모델 버전을 자세히 관리합니다. `관심` 탭에서는 관심 종목을 추가, 수정, 삭제할 수 있습니다.
