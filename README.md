...
웹은 `홈`, `계정`, `관심종목`, `모니터링`, `알림`, `투자전략`, `온톨로지`, `설정` 탭으로 구성됩니다. desktop에서는 왼쪽 고정 내비게이션, mobile에서는 하단 탭 내비게이션을 사용합니다. `계정` 탭은 로컬 SQLite DB에 저장된 계정 값을 폼에 채우고, secret 원문은 다시 표시하지 않습니다. `알림` 탭에서는 메시지 타입별 활성화, 주기, 임계값, 발송 채널, 마지막 발송 시각, 다음 발송 가능 시각, 타입별 메시지 템플릿을 관리하고, `투자전략` 탭에서는 모델 공식과 판단 기준을 관리합니다. `온톨로지` 탭은 TBox/ABox와 관계 그래프 기반 투자 의견을 별도로 보여줍니다.
...
온톨로지는 TBox/ABox로 나뉩니다. TBox는 투자 세계관의 클래스, 관계 타입, 추론 규칙이고, ABox는 현재 계좌 스냅샷에서 생성된 실제 보유, evidence, belief, opinion assertion입니다.
