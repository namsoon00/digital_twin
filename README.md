...
토스증권 Open API는 OAuth client secret과 계좌 헤더를 다루므로 Flutter Web에서 직접 호출하지 않습니다. 1단계 계약과 mock fixture는 `docs/toss-api-contract.md`와 `docs/fixtures/toss/`에 정리되어 있으며, 다음 단계에서는 로컬 BFF가 토큰 발급과 계좌 조회를 담당합니다.
