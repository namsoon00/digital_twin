(function () {
  ...
  var tabs = [
    ...
    { id: "calendar", label: "투자 캘린더", description: "이벤트·리마인더", groupId: "market" },
  ...
  var bottomTabIds = ["overview", "watchlist", "calendar", "notifications", "modeling"];
  ...
  var navigationGroups = [
    ...
    { id: "market", label: "데이터 관리", description: "계정·종목·근거 수집", tabIds: ["accounts", "watchlist", "calendar", "symbols", "feed"] },
  ...
  function serverSettingsPayload() {
    ...
    return {
      ...
      ontologyLabAutoApplyMinScore: settingValue("ontologyLabAutoApplyMinScore"),
  ...
  function currentInvestmentCalendar() {
    return state.investmentCalendar || {
      events: [],
  ...
  function loadInvestmentCalendar(force) {
    ...
    state.investmentCalendarLoading = true;
    ...
    return promise
      ...
      .catch(function (error) {
        state.investmentCalendarError = error.message || "투자 캘린더를 불러오지 못했습니다.";
      })
      .finally(function () {
  ...
  function recommendInvestmentCalendarCandidates() {
    if (state.investmentCalendarResearching) return Promise.resolve();
    var filters = state.investmentCalendarFilters || {};
    var payload = {
    ...
    state.investmentCalendarResearching = true;
    state.investmentCalendarError = "";
    render();
    var promise = isStaticPreviewHost()
      ? Promise.resolve({
    ...
    return promise
      .then(function (result) {
        state.investmentCalendarResearchResult = result || {};
      ...
      .catch(function (error) {
        state.investmentCalendarError = error.message || "AI 일정 후보를 추천하지 못했습니다.";
        showSnackbar(state.investmentCalendarError, "danger");
      })
      .finally(function () {
        state.investmentCalendarResearching = false;
        render();
      });
  }

  function saveInvestmentCalendarEvent() {
    if (state.investmentCalendarSaving) return Promise.resolve();
    ...
    state.investmentCalendarSaving = true;
    state.investmentCalendarError = "";
    render();
    return sendJson("/api/investment-calendar/events", "POST", payload)
      ...
      .catch(function (error) {
        state.investmentCalendarError = error.message || "투자 캘린더 이벤트를 저장하지 못했습니다.";
        showSnackbar(state.investmentCalendarError, "danger");
      })
      .finally(function () {
        state.investmentCalendarSaving = false;
        render();
      });
  }
  ...
  function deleteInvestmentCalendarEvent(eventId) {
    ...
    if (!id || state.investmentCalendarDeleting) return Promise.resolve();
    if (window.confirm && !window.confirm("선택한 투자 이벤트를 삭제할까요?")) return Promise.resolve();
    state.investmentCalendarDeleting = id;
    state.investmentCalendarError = "";
    render();
    return sendJson("/api/investment-calendar/events/" + encodeURIComponent(id), "DELETE", {})
      ...
      .catch(function (error) {
        state.investmentCalendarError = error.message || "투자 캘린더 이벤트를 삭제하지 못했습니다.";
        showSnackbar(state.investmentCalendarError, "danger");
      })
      .finally(function () {
        state.investmentCalendarDeleting = "";
        render();
      });
  }

  function runInvestmentCalendarReminders() {
    if (state.investmentCalendarRunning) return Promise.resolve();
    state.investmentCalendarRunning = true;
    state.investmentCalendarError = "";
    render();
    return sendJson("/api/investment-calendar/reminders/run", "POST", {})
      ...
      .catch(function (error) {
        state.investmentCalendarError = error.message || "캘린더 리마인더를 실행하지 못했습니다.";
        showSnackbar(state.investmentCalendarError, "danger");
      })
      .finally(function () {
  ...
  function renderActiveTab(snapshot) {
    ...
    if (state.activeTab === "calendar") {
      return renderInvestmentCalendarPage(snapshot);
    }
  ...
  function renderInvestmentCalendarSummaryPanel() {
    ...
    return [
      ...
      '<button class="text-button" type="button" data-action="research-investment-calendar"' + (state.investmentCalendarResearching ? ' disabled' : '') + '>' + (state.investmentCalendarResearching ? "리서치 중" : "AI 일정 추천") + '</button>',
  ...
  function renderInvestmentCalendarFormPanel(options) {
    ...
    return [
      ...
      '<button class="text-button primary" type="submit"' + (state.investmentCalendarSaving ? ' disabled' : '') + '>' + (state.investmentCalendarSaving ? "저장 중" : "이벤트 저장") + '</button>',
      '</div>',
      '</form>',
      '</article>'
    ].join("");
  ...
  function renderInvestmentCalendarEvent(event) {
    ...
    return [
      ...
      '<span>알림 ' + escapeHtml((event.reminderOffsetsMinutes || []).join(", ") || "없음") + '분 전</span>',
      '</div>',
  ...
  function renderInvestmentCalendarCandidatePanel() {
    ...
    return [
      ...
      '<div class="panel-head">',
      '<div><p class="label">AI RESEARCH QUEUE</p><h2>AI 추천/자동 감지 후보</h2><span>뉴스·공시 리서치에서 날짜 후보를 추천하고, 승인 전 단계로 보관합니다.</span></div>',
  ...
  function renderInvestmentCalendarResearchStatus() {
    var result = state.investmentCalendarResearchResult || {};
    var collection = result.collection || {};
    var hasResult = Object.keys(result).length > 0;
    return [
      '<div class="investment-calendar-research-strip">',
      '<div>',
      '<strong>' + escapeHtml(state.investmentCalendarResearching ? "AI 리서치 실행 중" : (hasResult ? "최근 AI 추천 결과" : "AI 리서치 대기")) + '</strong>',
      ...
      '</div>',
      '<button class="mini-button primary" type="button" data-action="research-investment-calendar"' + (state.investmentCalendarResearching ? ' disabled' : '') + '>' + escapeHtml(state.investmentCalendarResearching ? "실행 중" : "AI 추천 실행") + '</button>',
      '</div>'
    ].join("");
  }
  ...
  function renderInvestmentCalendarCandidate(candidate) {
    ...
    var reason = candidate.reviewReason === "missingDate" ? "날짜 확인 필요" : (candidate.reviewReason === "lowConfidence" ? "신뢰도 확인 필요" : (candidate.reviewReason === "aiResearchRecommended" ? "AI 추천 검토" : "검토 필요"));
    return [
  ...
  function renderTypeDBRuleboxPanel() {
    ...
    return [
      ...
      '<label class="setting-field"><span>반영 점수</span><input data-model-setting="ontologyLabAutoApplyMinScore" type="number" min="0" max="100" step="1" value="' + escapeHtml(settingValue("ontologyLabAutoApplyMinScore") || defaultSettings.ontologyLabAutoApplyMinScore || "75") + '"></label>',
  ...
  function renderFeedSettingsEditorPanel() {
    ...
    return [
      ...
      renderSettingsGroup("뉴스 아카이브", "저장형 뉴스 수집 워커의 대상과 속도입니다.", [
        ...
        renderSettingField("newsCollectionProviders", "뉴스 수집 채널", "text", "yahoo_finance,gdelt,google_rss_kr,google_rss_us"),
  ...
  function bindActions() {
    ...
    Array.prototype.slice.call(app.querySelectorAll("[data-calendar-field]")).forEach(function (field) {
      var updateCalendarDraft = function () {
        ...
        if (!name) return;
        state.investmentCalendarDraft[name] = field.value;
      };
    ...
    Array.prototype.slice.call(app.querySelectorAll("[data-calendar-filter]")).forEach(function (field) {
      var updateCalendarFilter = function () {
        ...
        if (!name) return;
        state.investmentCalendarFilters[name] = field.value;
      };
