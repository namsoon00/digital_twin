...
- 화면마다 같은 골격을 쓴다. PC는 좌측 고정 내비게이션과 우측 작업 영역, 태블릿은 상단 밀도형 내비게이션과 8컬럼 작업 영역, 모바일은 상단 운영 메뉴와 하단 핵심 탭이다.
...
- tablet `861px-1180px`에서는 좌측 사이드바를 쓰지 않는다. `app-nav`를 상단 sticky 바에 두고, 본문은 8컬럼 그리드로 구성해 노트북/태블릿에서 정보가 과하게 좁아지지 않게 한다.
...
- 태블릿 `861px-1180px`에서는 `admin-grid`를 8컬럼으로 줄이고, 비교가 필요한 패널만 2열을 유지한다. 860px 이하에서만 좁은 화면용 단일 흐름으로 접는다.
...
- tablet 탭은 상단 `.app-nav` 안에서 한 줄 스크롤 가능한 네비게이션으로 표시한다. 좌측 사이드바를 억지로 유지해 본문 폭을 희생하지 않는다.
...
- 태블릿 861-1180px에서 사이드바가 본문 폭을 갉아먹지 않고 상단 내비게이션 + 8컬럼으로 전환되는가
