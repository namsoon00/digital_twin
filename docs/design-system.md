...
- 모바일은 shell 외곽 14px, 섹션 gap 16px을 기본으로 한다. 좁은 화면에서도 패널, 버튼, 행 텍스트가 화면 가장자리나 하단 탭과 붙지 않아야 한다. 패널 헤더 바로 아래에 `model-guide-grid`, `process-rail`, `investment-bridge-flow`, 리스트형 body가 붙어 보이면 실패이며, 모바일 최종 spacing 레이어에서 상단 padding과 gap을 다시 보정한다.
