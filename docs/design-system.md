...
- Work detail layer: PC에서는 현재 화면 맥락을 벗어나는 긴 감사, 대형 그래프, 독립 검토에만 `.work-detail-layer`를 사용한다. 기사 본문, 채널 매트릭스, 품질 진단, 후보 판단 근거처럼 현재 행/패널과 직접 연결된 정보는 `inline-detail-surface`로 펼치고, 알림 판단처럼 목록 비교가 필요한 정보는 마스터-디테일 패널로 읽는다.
