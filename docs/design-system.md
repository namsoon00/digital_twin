...
전문 금융앱 같은 룩앤필은 장식이 아니라 안정감, 예측 가능성, 빠른 비교에서 나온다. Orbit Alpha는 Orbit Light 톤을 기본으로 쓰며, 브랜드 모티프는 중앙 코어, 얇은 궤도선, 작은 신호점으로 제한한다. 토스처럼 저채도 표면과 흰 카드 구조는 유지하되, 액션 블루, 시그널 그린, 딥 오빗 코어 컬러가 일관되게 보여야 한다.
...
- PC `981px+`에서는 관리 화면을 모바일 카드 확대판으로 만들지 않는다. 계좌/관심종목/홈/알림/전략처럼 비교와 편집이 함께 필요한 화면은 2열 이상을 유지하고, 980px 이하에서만 좁은 화면용 단일 흐름으로 접는다.
...
- 계정: PC에서는 저장 계정 목록과 편집 폼을 같은 화면에 노출한다. 저장 계정 행은 `account-exposure-grid`로 토스 API, 계좌 seq, 텔레그램, 관심종목 상태를 가로 비교할 수 있게 하고 API secret 원문은 표시하지 않는다. 저장/삭제는 계정 폼과 행 오른쪽 액션 위치를 따른다.
...
- PC 981px 이상에서 계좌/관심종목/홈/알림/전략 화면이 너무 일찍 1열로 접히지 않고 비교/편집에 맞는 2열 이상 구조를 유지하는가
