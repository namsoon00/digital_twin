...
- `nav.app-nav`는 브랜드, 전체 탭, 현재 데이터 상태, 새로고침을 한 줄에 묶는 최상단 앱 네비게이션이다.
- `section.topbar`는 현재 탭의 `h1`과 짧은 subtitle만 가진다. 상태/새로고침/관리 진입은 topbar 밖의 앱 네비게이션에서 처리한다.
- desktop `861px+`에서는 `app-nav`에 모든 탭을 표시하고, 하단 `tab-bar`는 숨긴다.
- mobile `860px 이하`에서는 100dvh 앱 셸처럼 보이게 한다. `app-nav`는 safe-area를 반영한 sticky 앱바이고, `tab-bar`는 화면 하단 edge에 붙은 핵심 내비게이션이다. 계정/전체종목/투자전략/설정은 `app-nav`의 `관리` 메뉴로 묶는다. 모바일 본문 그리드는 1열로 접고 가로 스크롤 탭을 만들지 않는다.
...
- `app-nav`는 모바일에서 `position: sticky`로 유지하고 배경 blur를 적용해 스크롤 중에도 현재 위치와 전역 이동을 잃지 않게 한다.
...
- desktop 탭은 `.app-nav` 안에서 라벨 중심의 네비게이션 바로 표시한다. 관리성 탭은 divider 뒤에 배치해 핵심 업무 탭과 시각적으로 구분한다.
- mobile primary 탭은 `홈`, `관심종목`, `모니터링`, `알림`을 하단 고정으로 둔다. 계정, 전체종목, 투자전략, 설정처럼 보조 빈도가 높은 화면은 상단 `관리` 메뉴에서 진입시킨다.
...
- 모바일에서 app-nav는 sticky, tab-bar는 bottom edge에 붙어 있고 safe-area와 100dvh 셸을 반영하는가
