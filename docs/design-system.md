...
- PC와 태블릿의 큰 `deskbar`는 홈 관제 화면에만 둔다. 계정/관심종목/전체종목/알림 판단/투자 판단/뉴스 영향/설정처럼 사용자가 작업하는 업무 탭에서는 상단 상태 카드 묶음을 렌더링하지 않는다.
- 모든 탭 본문은 `managed-page` 템플릿을 통과한다. 탭별로 임의의 카드 묶음을 만들지 말고, 업무 탭은 얇은 `page-command-strip-compact`로 화면 맥락, `이전 입력 -> 현재 처리 -> 다음 출력` flow spine, 핵심 지표만 한 줄에 둔 뒤 `page-routine-panel`과 실제 작업대를 바로 보여준다.
...
- 모든 탭은 `web-style-nav`, `web-style-topbar`, `web-style-workspace`, `web-style-main` 영역을 같은 순서로 렌더링한다.
- 홈은 `data-style-rail="full"` 데스크바를 쓴다. 나머지 업무 탭은 deskbar를 렌더링하지 않고, 데이터/포트폴리오/알림 상태는 좌측 nav 하단 상태와 compact command strip 지표로만 보조 표시한다.
...
- 홈의 `page-command-strip`은 현재 업무 그룹, 화면 레이어, 핵심 엔티티, 화면 목적, 3단계 업무 흐름과 핵심 지표를 모두 보여준다. 업무 탭의 `page-command-strip-compact`는 목적 설명과 단계 카드를 접고, 화면 맥락, `page-flow-spine`, 핵심 지표만 얇게 보여준다.
- `page-command-strip` 바로 아래에는 `page-routine-panel`을 둔다. 모든 주요 탭은 첫 화면에서 `현재 상태`, `왜 봐야 하나`, `다음 행동`을 같은 위치에 보여줘 사용자가 매일 같은 루틴으로 움직일 수 있어야 한다.
...
- desktop 탭은 좌측 `.app-nav` 안에서 업무 그룹 중심의 세로 내비게이션으로 표시한다. 그룹은 `Command`(홈 관제), `Market Desk`(계정·관심종목·전체종목·뉴스 영향), `Decision Stack`(투자 판단·알림 판단·전략 실험), `Control Plane`(시스템·설정) 순서다. 관리성 탭을 단순히 아래로 밀어 넣지 말고, 어느 업무 도메인의 입력/판단/운영에 속하는지 먼저 결정한다.
...
- Deskbar: PC/태블릿의 `.deskbar-full`은 홈 관제 화면에만 둔다. 업무 탭에는 `.deskbar-compact`를 만들지 않는다. 같은 상태 정보가 필요하면 좌측 nav 하단 상태, compact command strip 지표, 각 패널 내부 상태 셀에서만 제공한다.
- Managed page: 모든 탭은 `.managed-page` 안에 `.page-command-strip`과 `.admin-grid`를 둔다. 홈의 `page-command-strip`은 `page-command-context`(업무 그룹, 화면 레이어, 핵심 엔티티, 목적), 3단계 업무 흐름, 3개 핵심 지표를 표시한다. 업무 탭의 `page-command-strip-compact`는 한 줄 컨텍스트 바이며 `page-flow-spine`으로 `이전 입력`, `현재 처리`, `다음 출력`을 반드시 보여준다. 사용자가 실제로 봐야 할 첫 판단은 바로 아래 `page-routine-panel`이 맡는다.
...
- 알림 판단: 기본 진입은 `현황`이며 탭 스트립 바로 아래에 `notification-ops-rail`로 대기/발송/보류/실패/룰/주기/템플릿/스케줄을 한 줄 상태표로 보여준 뒤 최근 판단 목록을 전폭으로 배치한다. 최근 판단 항목은 제목보다 `점수 변화`, `기준/이전 대비 delta`, `상승·감점 요인`, `게이트와 보류 조건`을 먼저 보여준다. 긴 메시지는 본문 열에만 두며 상태/액션 열을 분리한다. `정책`, `템플릿`, `진단` 내부 섹션으로 나누고, 내부 섹션 이동은 카드형 버튼이 아니라 알림탭 최상단 탭 스트립으로 제공한다. 신호 섹션은 실행 상태/알림 센터 6:6, 통합 종목/계좌 노출 8:4로 배치한다. 고급 설정은 진단 섹션에 둔다.
...
- PC/태블릿에서 full deskbar는 홈에만 있고, 업무 탭은 deskbar 없이 compact command strip, flow spine, routine panel, 실제 콘텐츠 순서로 바로 내려가는가
