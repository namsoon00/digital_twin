...
- 투자 판단: 내부 섹션은 `오늘의 판단`, `투자 근거`, `통합 차트`, `전략 룰`, `온톨로지`, `검증·리뷰` 순서로 유지한다. 사용자는 `오늘의 판단 -> 투자 근거 -> 검증·리뷰` 3단계로 읽어야 하며, 룰 편집은 설정 모드에만 노출한다. 기본 진입인 오늘의 판단은 `상태 바`, `그룹형 액션 큐`, `선택 종목 요약`, `차단 요인` 네 덩어리만 렌더링한다. 자금 흐름 상세, AI 추론 입력 계약, 데이터 출처 원장, 온톨로지 그래프, 모델 재계산 상세는 각각 통합 차트·전략 룰·투자 근거·온톨로지·검증/리뷰 탭으로 보낸다. 투자 근거는 reasoning card와 실행 계획, 통합 차트는 가격·수급·자금흐름·매크로 신호의 같은 축 비교, 온톨로지는 TBox/ABox/InferenceBox 그래프와 `Relation Matrix`, 전략 룰은 설정 모드의 RuleBox semantic profile·TypeDB schema function sync·프롬프트 관리, 검증·리뷰는 재계산 결과와 데이터 품질·관계 추적을 담당한다.
