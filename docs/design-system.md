...
- 모든 탭 본문은 `managed-page` 템플릿을 통과한다. 화면 맥락, `이전 입력 -> 현재 처리 -> 다음 출력` flow spine, 핵심 지표, 결과/설정 전환, 현재 상태/이유/다음 행동은 상단 `app-nav-command` 하나로 승격하고 본문은 실제 작업대를 바로 보여준다. 모바일에서도 본문에 별도 command/routine 카드를 반복하지 않는다.
- PC 기본 화면은 전체 데이터를 다 펼치지 않는다. 첫 화면은 판단 가능한 원장/요약/상태만 보여주고, 짧은 근거·품질·런타임 진단은 `inline-detail-surface`, 입력 폼은 focused modal, 행 선택형 리포트는 마스터-디테일, 전체 감사/그래프처럼 화면 맥락을 벗어나는 긴 검토만 `work-detail-layer` 또는 별도 dialog/drawer로 연다.
- 상세 표면은 정보 성격에 맞게 고른다. 기사 본문, 투자 후보 근거, 수집 채널 전체, 데이터 품질처럼 현재 행/패널의 맥락에서 바로 읽어야 하는 정보는 그 자리에서 펼치고, 등록·편집 입력은 작은 모달로 분리하며, 알림 판단처럼 목록과 리포트를 비교해야 하는 정보는 같은 화면의 선택형 상세 패널로 둔다.
...
- 모든 탭은 `web-style-nav`, `web-style-topbar`, `web-style-workspace`, `web-style-main` 영역을 같은 순서로 렌더링한다. 보이는 화면 제목/맥락은 `web-style-nav` 안의 `app-nav-command`가 담당하고, `topbar`는 시맨틱 제목 영역으로만 남긴다.
...
- 모든 화면 본문은 `managed-page web-style-page`, `data-style-screen="<tab>"`, `data-structure-group`, `data-structure-layer`, `data-structure-entity`, `page-command-strip web-style-command-strip`, `admin-grid`를 통과한다. `page-command-strip`과 `page-routine-panel`은 DOM 계약으로만 유지하고, 보이는 정보는 `app-nav-command`에 합쳐져야 한다.
...
- `app-nav-command`는 현재 업무 그룹, 화면 레이어, 현재 탭, 화면 목적, `page-flow-spine`, 결과/설정 전환, 핵심 지표, `현재 상태`, `왜 봐야 하나`, `다음 행동`을 한 영역에 합친다.
- 모든 주요 탭은 첫 화면에서 사용자가 실제 작업대로 내려가기 전에 하나의 command rail만 통과해야 한다. 같은 역할의 `page-command-strip`과 `page-routine-panel`이 본문에 함께 보이면 실패다.
- 상세 정보는 기본 본문에 모두 펼치지 않되, 무조건 공통 레이어로 보내지 않는다. 1-2단계 확인은 `inline-detail-surface`, 짧은 입력은 modal, 행 선택 리포트는 마스터-디테일, 화면을 벗어나는 감사/그래프는 `work-detail-layer`나 dialog/drawer로 분기한다.
...
- 검증 대상 탭은 `overview`, `calendar`, `accounts`, `watchlist`, `symbols`, `notifications`, `modeling`, `experiments`, `feed`, `system`, `settings`다.
- 새 탭이나 새 화면은 먼저 이 계약의 DOM 구조와 CSS 레이어를 통과해야 하며, 예외가 필요하면 디자인 시스템 문서와 스모크 테스트를 함께 갱신한다.
- 투자 캘린더는 별도 `calendar` 탭의 월간 보드를 우선으로 한다. 추정 일정은 날짜 셀과 검토 큐에는 보이되, 승인 전에는 리마인더 대상이나 다음 알림으로 취급하지 않는다.
...
- `section.topbar`는 시맨틱 제목 영역이다. 화면에는 같은 정보가 `app-nav-command`로 합쳐지므로 시각적으로 숨긴다.
...
- desktop 탭은 상단 `.app-nav`에서 `오늘`, `시장`, `판단`, `알림`, `캘린더`, `설정`의 사용자 업무 순서를 유지한다. `근거 점검`은 판단 작업공간의 내부 화면이며 별도 관리 탭으로 중복 노출하지 않는다. PC에서 탭 라벨은 한 줄이어야 하며, 설명은 같은 `app-nav` 안의 `app-nav-command`가 담당한다.
...
- 판단은 `현재 의견`, `근거 점검`, `판단 기준`으로 나눈다. 알림은 의미 있는 변화와 전달 결과만 보여주고, 규칙·가설·실험·승격은 `설정 > 운영 관리 > 투자모델 관리`에 둔다.
- 861px 이상 tablet/desktop 탭은 모두 상단 `.app-nav` 안에서 한 줄 네비게이션으로 표시한다. 폭이 좁으면 탭 행만 가로 스크롤하고 본문 폭을 침범하지 않는다.
- mobile primary 탭은 `오늘`, `시장`, `판단`, `알림`, `캘린더`, `설정`을 하단 고정으로 둔다. 판단 내부 전환은 상단 3분할 컨트롤을 사용하고 운영 기능은 설정에서 진입한다.
...
- 주 네비게이션 탭은 각각의 페이지 스크롤 위치를 가진다. 한 화면 안의 내부 섹션 탭과 상세 탭은 전환 직전의 현재 뷰포트 위치를 유지하며 섹션별 위치로 이동하지 않는다. PC/태블릿과 모바일 모두 기본 스크롤 루트는 window/page이고, 탭 목록과 패널은 부모의 가로 폭을 늘릴 수 없다.
...
- Managed page: 모든 탭은 `.managed-page` 안에 `.page-command-strip`과 `.admin-grid`를 둔다. `page-command-context`, `page-flow-spine`, 핵심 지표, 결과/설정 전환은 `app-nav-command`로 합치고 본문 `page-command-strip`은 시각적으로 숨긴다. 사용자가 실제로 봐야 할 첫 판단도 `app-nav-command`의 routine 영역이 맡는다.
- Routine panel: `page-routine-panel`은 DOM 계약으로 남기되 시각적 표면은 만들지 않는다. `현재 상태`, `왜 봐야 하나`, `다음 행동`은 `app-nav-routine` 안에서 한 번만 보여준다.
- Work detail layer: PC에서는 현재 화면 맥락을 벗어나는 긴 감사, 대형 그래프, 독립 검토에만 `.work-detail-layer`를 사용한다. 목록 화면은 반투명 백드롭 뒤에 남기고 상세는 중앙 레이어로 띄운다. 일반 레코드는 `medium` 960px, 업무 보드는 `large` 1180px, 그래프·차트는 `wide` 1320px을 최대 폭으로 쓰며 92vw를 넘지 않는다. 레이어 본문에 자체 스크롤을 만들지 않고 백드롭 전체가 스크롤하며, 모바일에서만 전체 화면으로 전환한다. 조회 상세는 바깥 클릭과 `Escape`로 닫고, 저장 전 입력을 가진 편집 상세는 바깥 클릭으로 닫지 않는다. 기사 본문, 채널 매트릭스, 품질 진단, 후보 판단 근거처럼 현재 행/패널과 직접 연결된 정보는 `inline-detail-surface`로 펼치고, 알림 판단처럼 목록 비교가 필요한 정보는 마스터-디테일 패널로 읽는다.
- Card type taxonomy: PC의 내부 카드는 `data-card-type`으로 의미를 고정한다. `health-card`는 연결·인증·워커·신선도 같은 운영 상태, `metric-cell`은 숫자 요약, `ledger-row`는 계정·종목·설정 매핑 같은 반복 원장, `decision-row`는 알림 판단처럼 결론과 게이트가 있는 행, `calendar-event`는 실적·공시·거시지표처럼 시간 축에 놓이는 투자 이벤트, `evidence-card`는 뉴스·공시·근거 요약, `action-queue-card`는 운영자가 처리해야 할 다음 작업, `config-panel`은 설정 입력/저장 책임, `process-card`는 수집·분석·알림처럼 순서가 있는 단계, `source-card`는 API·워커·저장소·데이터 계보, `diagnostic-card`는 품질·준비도·검증 결과, `signal-card`는 가격·수급·모델·알림 후보 신호, `relationship-card`는 온톨로지 관계·규칙·추론 연결, `reference-card`는 매뉴얼·용어·운영 기준 같은 설명성 정보, `empty-state`는 데이터 없음·로딩·오류 상태에만 쓴다. 새 카드형 UI는 기존 `.panel`만 붙이지 말고 반드시 적절한 `data-card-type`을 함께 선언한다.
- Card format taxonomy: `data-card-format`은 카드가 어떤 표면으로 읽혀야 하는지 정의한다. `summary-list-card`는 카드 정보가 많을 때 리스트에서는 제목, 상태, 3-6개 핵심 지표, 상세 진입만 남기는 요약 표면이다. `decision-ticket`은 알림 판단·투자 후보처럼 결론, 확인 단계, 다음 행동이 있는 행에 쓰고 상세는 버튼이나 마스터-디테일/팝업으로 넘긴다. `document-card`는 뉴스·공시·본문 요약처럼 출처와 본문 근거가 있는 문서형 데이터에 쓰며, 기사 제목과 본문 요약을 분리한다. `market-ledger-row`는 종목·계좌·보유 상태처럼 숫자 비교가 필요한 원장 행에 쓴다. `control-strip`과 `pagination-strip`은 검색, 필터, 페이지 이동을 위한 조작 표면이다. PC에서는 의미 구분을 왼쪽 컬러 바에 의존하지 않고 톤 칩, 상단 hairline, 테두리 강도, 버튼 진입으로 표현한다.
...
- Page mode switch: `page-mode-switch`는 `app-nav-command` 안에 둔다. 이 스위치가 있는 화면은 기본 `결과` 모드에서 설정 폼과 고급 정책을 노출하지 않는다. 사용자가 설정 모드로 들어가야 저장/편집/고급 항목이 나타난다.
...
- 알림 운영: 기본 진입은 `현황`이며 탭 스트립 바로 아래에 `notification-ops-rail`로 대기/발송/보류/실패/룰/주기/템플릿/스케줄을 한 줄 상태표로 보여준 뒤 최근 판단 목록을 전폭으로 배치한다. 최근 판단 항목은 제목보다 `상태 변화`, `이전 대비 손익·가격 변화`, `새 근거`, `게이트와 보류 조건`을 먼저 보여준다. 긴 메시지는 본문 열에만 두며 상태/액션 열을 분리한다. `정책`, `템플릿`, `진단` 내부 섹션으로 나누고, 내부 섹션 이동은 카드형 버튼이 아니라 알림탭 최상단 탭 스트립으로 제공한다. 신호 섹션은 실행 상태/알림 센터 6:6, 통합 종목/계좌 노출 8:4로 배치한다. 고급 설정은 진단 섹션에 둔다.
- 알림 운영의 최근 판단 목록은 원장형 요약이다. 전체 메시지, 중복 판단 키, 상세 게이트, 모든 판단 근거는 행 안에 펼치지 않고 `상세 리포트` 레이어에서 보여준다.
- 투자 판단: 내부 섹션은 `오늘의 판단`, `투자 근거`, `통합 차트`, `전략 룰`, `온톨로지`, `전략 제안`, `검증·리뷰` 순서로 유지한다. 사용자는 `오늘의 판단 -> 투자 근거 -> 검증·리뷰` 3단계로 읽어야 하며, 룰 편집은 설정 모드에만 노출한다. 기본 진입인 오늘의 판단은 `상태 바`, `그룹형 액션 큐`, `선택 종목 요약`, `차단 요인` 네 덩어리만 렌더링한다. 자금 흐름 상세, AI 추론 입력 계약, 데이터 출처 원장, 온톨로지 그래프, 모델 재계산 상세는 각각 통합 차트·전략 룰·투자 근거·온톨로지·검증/리뷰 탭으로 보낸다. 투자 근거는 선택형 reasoning card 워크벤치로 한 종목씩 깊게 읽고, 통합 차트는 차트 기준선 패널 뒤에 가격·수급·자금흐름·매크로 신호를 같은 축으로 비교한다. 온톨로지는 TBox/ABox/InferenceBox/RuleBox 레이어를 선택해 그래프와 `Relation Matrix` 상세 레일을 분리하고, 전략 제안은 `요약`, `조건`, `검증`, `성과`, `이력` 내부 섹션으로 긴 폼과 검증 결과를 접는다. 검증·리뷰는 상단 상태판 뒤에 재계산 결과, 데이터 품질, 관계 추적 상세 진입점만 제공한다.
- 뉴스·근거: 기본 진입은 `영향 인박스`이며 기사 제목 목록이 아니라 본문 요약과 주가 영향 판단을 먼저 보여준다. 각 카드의 상단은 `호재/악재/중립`, 관련 종목, 출처·자료 상태이고, 본문은 한글 요약과 판단 이유이며, 기사 제목·출처·시간·원문 링크는 카드 하단 `기사` 영역에만 둔다. `근거 DB`는 같은 순서로 상세 근거를 펼치고, `수집원`은 채널/품질 상태만 담당한다. 뉴스 아카이브, 공시 원천, 중요도 게이트 같은 입력 정책은 `settings` 모드의 `피드 설정`으로 분리한다.
- 뉴스·근거의 기본 카드에는 요약과 영향 방향까지만 노출한다. 기사/근거의 전체 맥락, 수집 채널 전체, 품질 진단은 상세 레이어로 이동한다.
...
- PC/태블릿/모바일에서 full deskbar는 홈에만 있고, 업무 탭은 deskbar·본문 command strip·본문 routine panel 없이 `app-nav-command`, 실제 콘텐츠 순서로 바로 내려가는가
