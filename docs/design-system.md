...
- PC 기본 화면은 전체 데이터를 다 펼치지 않는다. 첫 화면은 판단 가능한 원장/요약/상태만 보여주고, 알림 리포트, 기사 본문, 수집 채널 전체, 데이터 품질, 투자 후보 근거, 런타임 연결 상세처럼 긴 정보는 `work-detail-layer` 레이어로 연다.
- 상세 레이어는 별도 화면 전환이 필요 없는 감사/확인용 표면이다. 본문 카드 안에 장문, 전체 메시지, 진단 목록, 긴 체크리스트를 계속 쌓지 않고 `상세`, `상세 리포트`, `수집원 상세`, `연결 상세` 같은 명령으로 분리한다.
...
- 상세 정보는 공통 `work-detail-layer`를 우선 사용한다. 새 긴 목록/본문/진단 UI를 만들 때 기본 본문에 모두 펼치지 말고 요약 행과 레이어 상세를 함께 설계해야 한다.
...
- Work detail layer: PC에서는 상세 검토가 필요한 긴 데이터에 `.work-detail-layer`를 사용한다. 기본 화면의 행이나 요약 카드에는 핵심 상태, 짧은 요약, 액션만 두고 장문 리포트, 기사 본문, 채널 매트릭스, 품질 진단, 후보 판단 근거는 레이어 안에서 읽는다.
