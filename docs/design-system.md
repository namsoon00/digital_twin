...
- 모든 탭 본문은 `managed-page` 템플릿을 통과한다. 탭별로 임의의 카드 묶음을 만들지 말고, 업무 탭은 얇은 `page-command-strip-compact`로 화면 맥락과 핵심 지표만 한 줄에 둔 뒤 `page-routine-panel`과 실제 작업대를 바로 보여준다.
...
- 홈의 `page-command-strip`은 현재 업무 그룹, 화면 레이어, 핵심 엔티티, 화면 목적, 3단계 업무 흐름과 핵심 지표를 모두 보여준다. 업무 탭의 `page-command-strip-compact`는 목적 설명과 단계 카드를 접고, 화면 맥락과 핵심 지표만 얇게 보여준다.
...
- Managed page: 모든 탭은 `.managed-page` 안에 `.page-command-strip`과 `.admin-grid`를 둔다. 홈의 `page-command-strip`은 `page-command-context`(업무 그룹, 화면 레이어, 핵심 엔티티, 목적), 3단계 업무 흐름, 3개 핵심 지표를 표시한다. 업무 탭의 `page-command-strip-compact`는 한 줄 컨텍스트 바이며, 사용자가 실제로 봐야 할 첫 판단은 바로 아래 `page-routine-panel`이 맡는다.
...
- PC/태블릿에서 full deskbar는 홈에만 있고, 업무 탭은 deskbar 없이 compact command strip, routine panel, 실제 콘텐츠 순서로 바로 내려가는가
