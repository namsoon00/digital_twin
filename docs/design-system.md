...
- PC와 태블릿에는 `deskbar`를 둔다. 데이터 모드, 포트폴리오, 모델 기준, 온톨로지 관계, 알림 상태를 매 화면 같은 위치에서 확인한다.
...
- Deskbar: PC/태블릿 화면의 `topbar` 아래에는 `.deskbar`를 둔다. 페이지별 내용과 무관하게 데이터 모드, 포트폴리오, 모델, 온톨로지, 알림 상태를 같은 순서로 표시한다.
- Managed page: 모든 탭은 `.managed-page` 안에 `.page-command-strip`과 `.admin-grid`를 둔다. `page-command-strip`은 3단계 업무 흐름과 3개 핵심 지표만 표시하고, 본문 패널을 대신 설명하지 않는다.
...
- 모니터링: PC에서는 상태 요약 4컬럼, 알림 센터 8컬럼, 보유·관심 통합 목록 8컬럼, 포트폴리오 노출 4컬럼으로 배치한다. 보유와 관심은 같은 `monitoring-instrument-row` 패턴으로 보여주고, 보유/관심 칩으로 상태를 구분한다.
...
- PC/태블릿에서 deskbar, process rail, formula ledger, relation matrix가 필요한 화면에 빠지지 않았는가
