...
- 모델링: 기본 진입에서 `Strategy Workflow`로 데이터 정합, 판단 항목 산출, 공식 평가, 기준 적용, 결과, 알림까지 순서를 보여준다. 판단 기준 섹션은 `Formula Ledger`로 모든 공식을 먼저 보여준 뒤 편집 폼과 버전 저장을 둔다. 판단 항목 설명, 재계산 결과는 dense grid를 쓰고 장식형 hero를 만들지 않는다.
- 관계 분석: 그래프보다 먼저 규칙 구조 -> 현재 데이터 -> 관계 -> 근거 -> 판단 근거 -> AI 의견 순서를 보여준다. `Relation Matrix`로 규칙 구조에만 있는 관계와 현재 데이터에서 실제 사용된 관계 수를 구분하고, 이후 관계 그래프와 테이블 저장 구조, 규칙 추적을 제공한다.
