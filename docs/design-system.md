...
- desktop에서는 어떤 탭도 100dvh 안에 갇힌 고정 화면으로 만들지 않는다. 좌측 네비게이션과 작업 영역은 유지하되, `shell`, `workspace-main`, `managed-page`, `admin-grid`는 페이지 스크롤을 따른다. 긴 목록도 사용자가 일반 페이지처럼 끝까지 스크롤할 수 있어야 하며, 내부 스크롤은 모달/큰 그래프 overlay처럼 명확한 예외에만 쓴다.
