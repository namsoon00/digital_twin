...
3. The model control plane evaluates affected predictive contracts once over the immutable `SharedPremiseWorld` graph and projects exact `ModelHypothesisEvidence`.
4. `typedb_ontology.py` stores the compact shared ABox, exact model evidence, and the private account overlay in TypeDB.
5. TypeDB schema functions combine model evidence with semantic, account, policy, quality, and execution facts and materialize generation-scoped InferenceBox entities, relations, and traces.
...
- V2 reasoning is world-partitioned. `SharedPremiseWorld` contains the account-free market, company, macro, disclosure, research, and other facts required by active contracts. The model control plane scores each affected market subject once and TypeDB emits compact shared-premise references. `PortfolioWorld` contains private position, P/L, exposure, policy, and execution facts plus only those premise references; it must not mirror raw quote, flow, news, macro, or company ABox rows or run market-model scoring a second time.
...
- Exact model-signal contracts are projected as `ModelSignalInterpretationPolicy`, not duplicated statistical calculations. Policies with the same `stock`, `holding`, or `watchlist` source context share one `ModelSignalBridge` schema function; the call query must still verify every policy's exact signal ID, model release, validation state, account predicates, and derivation lineage in TypeDB. See `docs/model-signal-interpretation-bridges.md`.
...
- The current adapter stores executable resolver, semantic, policy, quality, and execution profiles in TypeDB-compatible RuleBox rows. Model-signal policies reuse three source-context bridge functions while non-model rules retain their own physical functions; the current catalogue compiles 116 enabled logical contracts into 45 physical functions. The model scorer evaluates market-owned `any`/`optional` groups as part of the exact predictive contract. TypeDB evaluates remaining account and non-predictive groups and remains the sole owner of final relation and action-envelope materialization.
...
- Portfolio ontology projection must default to factual ABox plus registered model-evidence output. General local Python graph reasoning remains removed; every model-evidence candidate must still pass TypeDB schema-function sync/materialization before it can affect judgement.
