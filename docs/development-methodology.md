...
- The current adapter stores executable resolver, semantic, policy, quality, and execution rule profiles in TypeDB-compatible RuleBox rows, compiles them into TypeDB 3 schema functions, calls those functions from TypeQL, and materializes matched results back into TypeDB as InferenceBox output. The model scorer evaluates market-owned `any`/`optional` groups as part of the exact predictive contract. TypeDB evaluates remaining account and non-predictive groups and remains the sole owner of final relation and action-envelope materialization.
