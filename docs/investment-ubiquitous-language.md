...
내부 추론과 사용자 설명은 같은 데이터에서 나오지만 역할과 언어를 분리한다. 내부 저장소에는 규칙, 가설, 근거 경로, 상태 전이를 원래 용어로 보존하고, 알림과 웹 요약에는 `customer-investment-document-v1` 문서만 표시한다.

- 규칙 기반 변화 감지는 가격·거래량·외국인·기관 매매 흐름·뉴스·공시 가운데 무엇이 달라졌는지 알려준다. 매수·매도 결론을 대신 만들지 않는다.
- AI 종합 판단은 규칙 기반 후보와 반대 근거를 비교해 지금 할 일, 하지 말아야 할 일, 판단이 바뀌는 조건을 알려준다.
- AI 호출 없이 안전한 로컬 폴백이 판단을 만들면 `규칙 기반 종합 판단`으로 표시한다. 이 결과를 AI 판단으로 표시하지 않는다.
- `TypeDB`, `TBox`, `ABox`, `RuleBox`, `InferenceBox`, 추론 세대, 가설, 인과 경로, 관계 수명주기 같은 말은 감사 화면에서만 사용한다.
- 사용자 문서에는 가격 흐름, 외국인·기관 매매 흐름, 실적과 재무 상태, 현재 가격 수준, 금리·환율 환경처럼 실제 투자 자료를 가리키는 말을 사용한다.
- 사용자 문서는 `customer-investment-document-quality-v1` 검사를 거치며 역할, 첫 결론, 다음 알림, AI 행동 항목과 내부 용어 누출 여부를 기록한다.
- 시스템이 자동으로 확인한다고 표시하는 항목은 구조화된 `followUpConditions`로 저장된 활성 조건이어야 한다.
- 문장으로만 남은 `nextChecks`는 시스템 자동 추적으로 표시하지 않고 추가로 필요한 외부 자료로 구분한다.
- 추적 조건이 충족되면 `investment.follow_up_transitioned` 사건을 발행해 새 규칙 분석을 시작한다. 투자 행동 후보가 생기면 별도 AI 종합 판단으로 연결한다.
- 새 판단 에피소드가 저장되면 이전 판단의 미충족 조건은 `superseded`로 종료한다. 과거 조건은 감사 이력으로 보존하되 현재 감시 대상으로 사용하지 않는다.
