...
- 투자 판단은 점수나 확률이 아니라 **확인 단계**, **자료 상태**, **변화 상태**, **근거 역할**로 표현한다.
...
| 확인 단계 | 지금 관계와 근거를 다시 볼 필요의 단계 | 예측 확률, 매도 확률, 점수 |
| 자료 상태 | 판단에 필요한 자료가 충분한지 여부 | 데이터 신뢰도 점수 |
| 변화 상태 | 이전 알림과 비교해 새 조건·개선·악화가 있었는지 | 변화 점수 |
| 추론 상태 변경 | 직전 최종 판단과 비교해 권장 대응·확인 단계·자료 상태·AI 검증 상태가 바뀐 것 | 그래프 세대 변경, 점수 변경 |
| 근거 역할 | 위험, 버티는 근거, 반대 근거, 참고, 판단 차단 중 어떤 역할인지 | 근거 가중치 |
