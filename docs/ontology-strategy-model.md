...
AI가 만든 신규 가설은 `hypothesis_development_cases`에서 제안 계보를 유지한다. 기존 TBox와 지원 모델을 사용하는 예측 후보는 기술 검증 후 별도 TypeDB 릴리스에서 독립 미래 결과를 비교하고 정책과 실행 검증을 모두 통과하면 자동 채택한다. 운영 규칙에 직접 덮어쓰지 않는다. 과거 스냅샷 수나 현재 조회 성공은 실증 검증이 아니다. 기존 수동 승인 경로와 자동 진화 경로를 구분하며 세부 범위·정책·복구 절차는 [온톨로지 자동 진화](ontology-evolution.md)에 정리한다.
