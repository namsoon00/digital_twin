...
export function renderDecisionReview(review, formatClock = value => String(value || "")) {
  var value = review || {};
  var labels = { evaluated: "결과 확인", "data-gap": "자료 보충 대기", partial: "일부 확인", excluded: "평가 제외", pending: "관측 대기", "not-scheduled": "관측 예약 없음", unavailable: "관측 상태 확인 불가", "not-recorded": "이전 기록 없음" };
  ...
  return '<section class="oa-decision-review" data-review-state="' + escapeHtml(value.state || "not-recorded") + '">' +
    ...
    '<div><dt>사후 검증</dt><dd>' + (value.scheduleExplanation ? '<p>' + escapeHtml(value.scheduleExplanation) + '</p>' : '') +
    (value.nextObservationAt ? '<time>다음 관측 ' + escapeHtml(formatClock(value.nextObservationAt)) + '</time>' : '') + (outcomes.length ? '<ul>' + outcomes.slice(-3).map(function (item) {
    ...
    }).join("") + '</ul>' : (value.scheduleExplanation ? '' : value.state === "pending" ? '정해진 관측 시점의 결과를 기다립니다.' : '저장된 검증 결과가 없습니다.')) + '</dd></div>' +
