...
function renderAlertsConsole() {
  ...
  var metrics = [
    ...
    { label: "확인 필요", value: Number(inbox.actionRequired || 0) + "건", detail: "전달 실패·미확인 항목 포함", tone: "neutral", target: { type: "notification", value: "action" } },
  ...
  return renderConsoleManagedPage("notifications", metrics, [
    ...
    renderConsoleSurface({ title: "변화 알림", actions: '<button class="text-button compact" type="button" data-action="mark-all-notifications-read">모두 읽음</button>', body: renderSecondaryDisclosure("alerts-filters", "검색·필터", toolbar, notificationsState.notificationJobSearch || "전체 알림 · 최신순") + renderConsoleLiveRegion("alerts-ledger-body", table), footer: renderNotificationJobPager() }),
    renderWorkDetailButton("notification-policy-board", "", "알림 수신·발송 정책", "text-button compact"),
    '</div>'
  ].join(""), { loading: initialLoading, secondaryMetrics: true });
}
