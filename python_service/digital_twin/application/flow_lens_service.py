...
def toss_decision_for_holding(item: Dict[str, object], portfolio: Dict[str, object], strategy_model: StrategyModel = None) -> Dict[str, object]:
    ...
    foreign_buy = number(item.get("foreignBuyVolume"))
    foreign_sell = number(item.get("foreignSellVolume"))
    institution_buy = number(item.get("institutionBuyVolume"))
    institution_sell = number(item.get("institutionSellVolume"))
    individual_buy = number(item.get("individualBuyVolume"))
    individual_sell = number(item.get("individualSellVolume"))
    decision_position = Position(
        ...
        foreign_net_volume=investor_net_volume(item.get("foreignNetVolume"), foreign_buy, foreign_sell),
        foreign_net_amount=number(item.get("foreignNetAmount")),
        institution_buy_volume=institution_buy,
        institution_sell_volume=institution_sell,
        institution_net_volume=investor_net_volume(item.get("institutionNetVolume"), institution_buy, institution_sell),
        ...
        individual_net_volume=investor_net_volume(item.get("individualNetVolume"), individual_buy, individual_sell),
...
def ontology_relation_priority(payload: Dict[str, object]) -> float:
    ...
    priority_map = {
        ...
        "HAS_LIQUIDITY_PROFILE": 75,
        "HAS_EXECUTION_METRIC": 75,
        "LIMITED_BY_LIQUIDITY": 75,
        "HAS_EXIT_CAPACITY": 75,
        "HAS_EXECUTION_CAPACITY": 75,
