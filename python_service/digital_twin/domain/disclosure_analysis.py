...
def local_disclosure_analysis(context: Dict[str, object], source: str = "로컬 규칙") -> DisclosureAnalysisResult:
    ...
    if any(term in normalized for term in ["단일판매", "공급계약", "수주"]):
        ...
        meaning = "자사주 취득·처분·소각 등 주주환원이나 수급 변화 관련 공시입니다."
        impact = "취득·소각은 수급에 우호적일 수 있고 처분은 단기 물량 부담이 될 수 있습니다."
        check = "취득인지 처분인지, 규모, 기간, 목적, 소각 여부를 확인하세요."
        action = "취득·소각이면 긍정 요인을 반영하되 실제 집행률을 추적하세요."
