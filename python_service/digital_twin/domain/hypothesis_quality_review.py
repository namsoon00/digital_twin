...
def quality_review_for_item(item: Mapping[str, object]) -> Dict[str, object]:
    ...
    return {
        ...
        "predictionTarget": text(outcome.get("predictionTarget") or source.get("predictionTarget")),
        "expectedDirection": text(outcome.get("expectedDirection") or source.get("expectedDirection")),
        "expectedOutcome": text(outcome.get("expectedOutcome") or source.get("expectedOutcome")),
        "outcomeMetric": text(outcome.get("outcomeMetric") or source.get("outcomeMetric")),
        "falsificationContract": text(outcome.get("falsificationContract") or source.get("falsificationContract")),
        "horizonAssessments": [dict(item) for item in outcome.get("horizonAssessments") or [] if isinstance(item, Mapping)],
        "qualityState": state,
