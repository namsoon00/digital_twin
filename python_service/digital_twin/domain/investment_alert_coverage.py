...
REASONING_DELIVERY_TRIGGER_VERSION = "reasoning-delivery-trigger-v1"
...
def reasoning_delivery_trigger(
    source_events: Iterable[Mapping[str, object]],
    symbol: str,
    *,
    changed_fields_by_symbol: Mapping[str, object] = None,
) -> Dict[str, object]:
    """Preserve a verified source transition without assigning investment meaning.

    The reasoning request may be generated by frequent provider refreshes. Only
    an exact observation follow-up or a source materiality assessment may cross
    this boundary. TypeDB still owns the relation and AI still owns the user
    interpretation; this contract only explains why the current generation ran.
    """
    ...
    changed_fields = {
        _text(item)
        for item in _mapping(changed_fields_by_symbol).get(clean_symbol) or []
        if _text(item)
    }
    for raw_event in source_events or []:
        ...
        event_symbols.update({
            _upper(item.get("subject") or item.get("symbol"))
            for item in payload.get("materialityAssessments") or []
            if isinstance(item, Mapping)
            and _upper(item.get("subject") or item.get("symbol"))
        })
        event_symbols.update({
            _upper(item)
            for item in payload.get("observationFollowupSymbols") or []
        ...
        matched_rows.append({
            "kind": (
                ...
                else "verified-material-source-transition"
            ),
            "reason": reason,
            "matchedConditions": sorted(matched_conditions),
            "sourceEventId": event_id,
            "sourceEventName": _text(
                event.get("name") or payload.get("sourceEventName")
            ),
    ...
    revision_keys = sorted({
        _text(item.get("sourceRevision")) or _text(item.get("sourceEventId"))
    ...
    return {
        ...
        "sourceEventIds": sorted({
            ...
            for item in matched_rows
            if _text(item.get("sourceEventId"))
        }),
        "sourceEventNames": sorted({
            _text(item.get("sourceEventName"))
            for item in matched_rows
            if _text(item.get("sourceEventName"))
        }),
        "observedAt": max(
            [_text(item.get("observedAt")) for item in matched_rows]
            or [""]
        ),
        "observationFollowup": any(
            bool(item.get("observationFollowup")) for item in matched_rows
        ),
    }
