...
def normalize_position(item: Dict[str, object]) -> Position:
    ...
    return Position(
        ...
        freshness_status=str(item.get("freshnessStatus") or item.get("freshness_status") or ""),
        freshness_reason=str(item.get("freshnessReason") or item.get("freshness_reason") or ""),
        freshness_age_minutes=optional_number(item, ["freshnessAgeMinutes", "freshness_age_minutes"]),
        freshness_max_age_minutes=optional_number(item, ["freshnessMaxAgeMinutes", "maxAgeMinutes", "freshness_max_age_minutes"]),
        latency_status=str(item.get("latencyStatus") or item.get("latency_status") or ""),
        latency_reason=str(item.get("latencyReason") or item.get("latency_reason") or ""),
        market_session=str(item.get("marketSession") or item.get("market_session") or ""),
        market_session_label=str(item.get("marketSessionLabel") or item.get("market_session_label") or ""),
        source_transport=str(item.get("transport") or item.get("sourceTransport") or item.get("source_transport") or ""),
        real_time=bool(item.get("realTime") or item.get("real_time")),
