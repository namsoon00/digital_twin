...
from digital_twin.domain.crypto_market_signals import CRYPTO_TRANSITION_BASELINE_METADATA_KEY, crypto_transition_baseline
from digital_twin.domain.events import DomainEvent, EXTERNAL_FACT_CHANGED, ONTOLOGY_REASONING_REQUESTED, RESEARCH_EVIDENCE_COLLECTED, alerts_detected_event, monitoring_cycle_completed_event, snapshot_collected_event
from digital_twin.domain.message_types import MARKET_OBSERVATION
from digital_twin.domain.market_observations import apply_market_observation_outbox_baselines, apply_market_observation_reasoning_baselines, hydrate_market_observation_baselines, market_observation_delivery_admission, market_observation_delivery_cadence_minutes, market_observation_reasoning_candidates, market_observation_reasoning_symbols
from digital_twin.domain.market_signal_transitions import prepare_market_signal_transition_metadata
from digital_twin.domain.ontology_projection_input import compact_monitor_state_for_ontology, compact_monitor_state_for_reasoning_base, compact_monitor_state_for_reasoning_symbol, reasoning_snapshot_symbols
from digital_twin.domain.portfolio import AccountSnapshot, AlertEvent, account_snapshot_from_monitor_state, monitor_state_has_live_account_data
...
from digital_twin.infrastructure.mysql_operational_events import domain_event_from_row, insert_domain_event_with_connection
from digital_twin.modules.reasoning.infrastructure.mysql_reasoning_ingress import ingress_reasoning_event_with_connection
from digital_twin.infrastructure.mysql_operational_helpers import _json_loads, _sent_key_hash


from digital_twin.modules.notifications.infrastructure.mysql_notification_config import MySQLNotificationTemplateStore
from digital_twin.infrastructure.mysql_market_stores import MySQLModelReviewJobStore
...
class MySQLMonitorStore(MySQLOperationalConnection):
    ...
    def record_cycle(
        ...
        from digital_twin.infrastructure.time_series_factory import build_versioned_time_series_store
...
class MySQLEventLog(MySQLOperationalConnection):
    ...
    def external_fact_events_after(
        self,
        after_occurred_at: str = "",
        after_event_id: str = "",
        limit: int = 100,
    ) -> List[DomainEvent]:
        """Read durable external fact changes for official-evidence projection."""
