...
from digital_twin.modules.portfolio.infrastructure.mandate_store import save_mandate_with_connection
...
class MySQLInvestmentDomainStore(MySQLOperationalConnection):
    ...
    def append_ledger_entries_with_connection(self, connection, entries, stamp: str) -> int:
        return portfolio_writes.append_ledger_entries(
    ...
    def commit_snapshot_observation(
        ...
        def commit(connection):
            ...
            if activity_episode:
                ...
                portfolio_writes.insert_activity_episode(
                    connection=connection,
            ...
            portfolio_writes.insert_state_snapshot(
                connection=connection,
                stamp=stamp,
            ...
            portfolio_writes.insert_observation_reconciliation(
                connection=connection,
                reconciliation=reconciliation,
                reconciliation_payload=reconciliation_payload,
                stamp=stamp,
            )
            portfolio_writes.insert_observation_exposure(
                connection=connection,
                exposure=exposure,
                stamp=stamp,
            )
            ...
            if rebalance_proposal:
                portfolio_writes.upsert_observation_rebalance(
                    connection=connection,
                    rebalance_proposal=rebalance_proposal,
                    stamp=stamp,
                )
            cycle_payload = decision_cycle.to_dict()
            portfolio_writes.upsert_observation_cycle(
                connection=connection,
            ...
            for observation in action_rows:
                ...
                portfolio_writes.insert_action_observation(
                    connection=connection,
            ...
            portfolio_writes.advance_observation_checkpoint(
                connection=connection,
    ...
    def save_risk_snapshot_with_connection(
        ...
        return portfolio_writes.save_risk_snapshot(
            connection=connection,
            snapshot=snapshot,
            stamp=stamp,
        )
    ...
    def save_execution_episode_with_connection(self, connection, episode: ExecutionEpisode, stamp: str) -> None:
        return portfolio_writes.save_execution_episode(
            connection=connection,
            episode=episode,
            stamp=stamp,
        )
    ...
    def save_decision_review_with_connection(self, connection, review: DecisionReview, stamp: str) -> None:
        return outcomes_writes.save_decision_review(
            connection=connection,
            review=review,
            stamp=stamp,
        )
    ...
    def save_performance_attribution_with_connection(
        ...
        return outcomes_writes.save_performance_attribution(
            connection=connection,
