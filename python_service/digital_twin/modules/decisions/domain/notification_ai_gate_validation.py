...
from digital_twin.modules.notifications.contracts import message_delivery_profile, normalize_message_delivery_level
from digital_twin.modules.news_intelligence.contracts import active_company_valuation_rule_ids
from digital_twin.modules.decisions.domain.decision_evidence_contract import cap_decision_readiness, decision_eligible_hypothesis_payload, decision_readiness_contract, hypothesis_decision_eligibility, hypothesis_set_evidence_summary, material_action_transition_contract, minimum_hypothesis_comparison_count, temporal_evidence_summary
from digital_twin.modules.outcomes.contracts import normalize_follow_up_conditions
from digital_twin.modules.decisions.domain.investment_brain import hypothesis_comparison_audit, is_research_reviewable_hypothesis_payload, is_selectable_hypothesis_payload
from digital_twin.modules.decisions.domain.investment_decision_history import previous_decision_episode_value
from digital_twin.modules.decisions.domain.investment_insight_assessment import investment_insight_assessment
from digital_twin.modules.decisions.domain.investment_strategy_guidance import merge_strategy_context, strategy_guidance_context
from digital_twin.modules.decisions.domain.notification_ai import active_investment_opinion_value, build_notification_ai_opinion, criterion_lines, has_graph_backed_relation_context, missing_data_labels, notification_ai_prompt_context, relation_context_value
from digital_twin.modules.decisions.domain.notification_ai_gate_contracts import ACTION_LABELS, AI_DECISION_MODE, VALID_ACTIONS, NotificationAIValidatedResponse
from digital_twin.modules.decisions.domain.notification_ai_gate_sources import append_unique_source_url, select_source_urls_for_message, source_labels_from_context, source_urls_from_context
from digital_twin.modules.decisions.domain.notification_ai_gate_text import _line_after_colon, _raw_lines, _text, append_unique_text, fallback_action_from_label, parse_ai_response_json, precomputed_action_value, reference_date, soften_order_language, user_friendly_ai_list, user_friendly_ai_text
from digital_twin.modules.decisions.domain.notification_ai_context import is_watchlist_context, target_position_role
from digital_twin.modules.notifications.contracts import normalize_narrative_claims, response_writer_provenance
from digital_twin.modules.reasoning.contracts import ACTION_ENVELOPE_STATUS_LABELS, DATA_STATE_LABELS, REVIEW_LEVEL_LABELS, VALIDATION_STATE_LABELS, review_level_for, validation_state_for
from digital_twin.modules.model_registry.contracts import WATCHLIST_ACTION_POLICY
...
def watchlist_friendly_text(context: Dict[str, object], value: object) -> str:
    ...
    if not text or not is_entry_only_action_context(context or {}):
        return text
    replacements = [
        ("HOLD에서 AVOID", "관심 유지에서 신규 진입 회피"),
        ("HOLD에서 BUY", "관심 유지에서 소액 진입 검토"),
        ("HOLD에서", "관심 유지에서"),
        ("AVOID로", "신규 진입 회피로"),
        ("BUY로", "소액 진입 검토로"),
...
def build_notification_ai_gate_prompt(
    ...
    lines = [
        ...
        "내부 비교와 감사에는 TypeDB, TBox, ABox, RuleBox, InferenceBox, 가설, 인과 경로, 수명주기, materialization 같은 정확한 기술 개념을 사용해도 된다. 그러나 summary, opinion, currentActionPlan, changeAnalysis, nextActionPlan, evidence, counterEvidence, invalidationCondition, nextChecks, narrativeClaims의 text와 insightAssessment의 사용자 문장에는 이 구현 용어를 쓰지 않는다.",
        "사용자 문장에서는 내부 구조 이름 대신 실제로 함께 본 데이터를 말한다. '관계·가설·인과 경로'는 가격·거래량·외국인·기관 매매 흐름·뉴스·공시·실적·금리·환율 중 무엇을 함께 봤고 어떤 의미인지로 풀어 쓴다. '수급'은 가능하면 '외국인·기관 매매 흐름', '추세'는 '가격 흐름', '밸류에이션'은 '현재 가격 수준', '펀더멘털'은 '실적과 재무 상태'로 쓴다.",
        ...
        "입력에 재현 가능한 수치형 observableFollowUpFields가 있으면 무효화·약화·전환 조건을 followUpConditions의 weaken, invalidate 또는 switch로 구조화한다. 현재 입력으로 구조화할 수 있는 모든 수치형 다음 조건을 옮기고 nextChecks에 같은 조건을 중복하지 않는다. 입력에 없는 수치나 임계값은 만들지 않는다.",
        "시스템이 자동으로 확인한다고 표현할 조건은 반드시 followUpConditions에 field, operator, threshold, purpose, label, onSatisfied를 모두 기록한다. 구조화하지 못한 nextChecks는 자동 추적이라고 말하지 말고 아직 필요한 외부 자료와 갱신 출처를 구체적으로 쓴다.",
