...
from digital_twin.modules.notifications.contracts import compact_number, money, pct_delta, price_money, signed_pct, trade_strength_label
from digital_twin.modules.market_data.domain.market_data import investor_net_volume, number
...
class MonitoringPositionContextMixin:
    ...
    def position_value_base(self, position: Dict[str, object]) -> float:
        ...
        account_base_value = (
            number(position.get("account_value_krw"))
            or number(position.get("accountValueKrw"))
        )
        if account_base_value > 0:
            return account_base_value
