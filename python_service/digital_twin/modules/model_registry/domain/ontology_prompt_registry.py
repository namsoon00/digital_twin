...
from digital_twin.modules.model_registry.domain.ontology_relation_contracts import AI_PROMPT_REGISTRY_VERSION, OntologyPromptTemplate
from digital_twin.modules.model_registry.domain.ontology_relation_catalog import DEFAULT_RELATION_RULES
...
DEFAULT_PROMPT_TEMPLATES = [
    ...
    _prompt(
        ...
        guardrails=[
            "확인 단계, 자료 상태, 관계 규칙, 뉴스·공시, 포트폴리오 노출을 종합해 하나의 투자 의견을 고릅니다.",
    ...
    _prompt(
        ...
        "진입 조건의 성립 상태, 현재가, 수급, 추세, 적정가 정보를 보고 분할매수 가능성과 보류 조건을 나눠 설명한다.",
    ),
    _prompt(
        "modelSell",
        "매도 후보 AI 의견",
        "모델 매도 후보 알림의 매도 압력과 분할 대응 기준을 설명합니다.",
        "손실 관리 조건의 성립 상태, 손익률, 수급, 추세를 보고 분할매도, 손절, 보유 유지 중 어떤 확인 기준이 우선인지 설명한다.",
    ),
    _prompt(
        "watchlistBuyCandidate",
        "관심종목 매수 후보 AI 의견",
        "보유 전 관심종목의 매수 후보 신호를 진입 조건과 보류 조건으로 분리합니다.",
        "관심종목의 가격, 거래량, 추세, 진입 조건 상태를 보고 첫 진입 전 확인할 조건을 제시한다.",
    ),
    _prompt(
        "watchlistQuote",
        "관심종목 시세 AI 의견",
    ...
    _prompt(
        ...
        "InferenceBox 상태, 관계 수, 근거 수, 보유 종목 수, 차단 사유를 보고 매수·매도 판단을 만들지 않은 이유와 TypeDB, 네이티브 규칙, 추론 워커 점검 순서를 제시한다.",
        ...
        guardrails=[
            ...
            "TypeDB 연결, 네이티브 규칙 저장, 온톨로지 추론 워커 순서로 점검 항목을 제시합니다.",
        ],
    ),
    _prompt(
        "monitorHeartbeat",
    ...
    _prompt(
        ...
        guardrails=[
            ...
            "권장 행동이 같아도 성립한 조건과 근거 역할의 변화를 분리합니다.",
        ],
    ),
    _prompt(
        "externalEquityMove",
...
def default_ai_prompt_policy_text() -> str:
    return "\n".join([
        ...
        "useCategoricalDecisionStates=1",
    ])
