...
from digital_twin.modules.news_intelligence.domain.disclosure_quality import normalize_official_document_text
from digital_twin.modules.news_intelligence.domain.disclosure_taxonomy import classify_disclosure

DISCLOSURE_ANALYSIS_PROMPT_VERSION = "disclosure-analysis-v6-title-first"
...
def local_disclosure_analysis(context: Dict[str, object], source: str = "로컬 규칙") -> DisclosureAnalysisResult:
    ...
    title_key = report.replace("ㆍ", "·").replace(" ", "")
    normalized = (report + " " + document_text[:8000]).replace("ㆍ", "·").replace(" ", "")
    taxonomy = classify_disclosure(
        report,
        report,
        disclosure.get("provider"),
        document_text,
    )
    ...
    if any(term in title_key for term in ["임원·주요주주", "주식등의대량보유", "최대주주등소유주식변동"]):
        meaning = "임원 또는 주요주주의 보유 주식 변동을 알리는 공시입니다."
        impact = "내부자의 매수·매도 방향은 참고 신호지만, 이 공시 자체가 신주 발행이나 주식 가치 희석을 뜻하지는 않습니다."
        check = "변동 전후 보유 주식 수와 지분율, 매수·매도 여부, 변동 사유를 확인하세요."
        action = "한 건만으로 매매하지 말고 같은 내부자의 반복 매수·매도와 지분율 변화를 추적하세요."
    elif any(term in title_key for term in ["사업보고서", "분기보고서", "반기보고서"]):
        meaning = "매출·이익·현금흐름과 주요 사업 위험을 정기적으로 보고한 공시입니다."
        impact = "이전 보고 기간과 시장 예상보다 실적이 좋아졌는지에 따라 기업가치 평가가 달라질 수 있습니다."
        check = "매출, 영업이익, 영업현금흐름, 부채와 주식 수가 이전 기간보다 어떻게 변했는지 확인하세요."
        action = "핵심 수치의 방향이 기존 판단과 달라졌을 때 보유 비중과 매수 조건을 다시 점검하세요."
    elif any(term in title_key for term in [
        "특수관계인에대한", "대규모내부거래", "관계회사와의거래",
        "동일인등출자계열회사와의상품·용역거래",
    ]):
        meaning = "계열사나 주요 관계자와의 거래 조건을 공개한 공시입니다."
        impact = "거래 규모와 가격이 통상 조건에서 벗어나면 회사 현금흐름이나 주주가치에 부담이 될 수 있습니다."
        check = "거래 상대방, 금액, 산정 기준, 최근 매출·자산 대비 비중과 이사회 승인 내용을 확인하세요."
        action = "거래 규모가 크거나 조건이 불리할 때 지배구조 위험으로 반영하고 후속 공시를 추적하세요."
    elif "주식소각" in title_key or ("자기주식" in title_key and "소각" in title_key):
        meaning = "보유 자기주식을 없애 발행 주식 수를 줄이기로 한 공시입니다."
        impact = "실제 소각되면 남은 주식의 주당 가치와 주주환원 측면에 우호적일 수 있습니다."
        check = "소각 주식 수와 전체 주식 대비 비율, 소각 예정일, 이미 보유한 자사주인지 확인하세요."
        action = "소각 규모와 완료 여부를 확인한 뒤 주주환원 강도의 변화로 반영하세요."
    elif "자기주식" in title_key or "자사주" in title_key:
        if "처분" in title_key:
            ...
            action = "처분 방식과 규모가 확인될 때까지 추가매수보다 실제 시장 물량 영향을 먼저 보세요."
        else:
            meaning = "회사가 자기주식을 취득하기로 한 주주환원·수급 관련 공시입니다."
            impact = "실제 매입이 진행되면 주가 수급과 주주가치에 우호적일 수 있지만 결정만으로 집행이 보장되지는 않습니다."
            check = "취득 수량과 금액, 전체 주식 대비 비율, 기간, 목적과 실제 집행률을 확인하세요."
            action = "결정 발표보다 후속 취득 내역과 집행률이 이어지는지 추적해 긍정 강도를 판단하세요."
    elif any(term in title_key for term in ["유상증자", "전환사채", "신주인수권", "교환사채"]):
    ...
    elif any(term in title_key for term in ["합병", "분할", "영업양수", "영업양도"]):
        ...
        action = "이벤트 완료 전 변동성이 커질 수 있어 비중과 손실 제한 기준을 먼저 정리하세요."
    elif any(term in title_key for term in ["시설투자", "신규시설", "생산시설"]):
        meaning = "생산 능력이나 사업 기반을 늘리기 위한 투자 계획 공시입니다."
        impact = "수요가 뒷받침되면 성장 기반이 되지만 투자비 증가와 가동 지연은 현금흐름 부담이 될 수 있습니다."
        check = "투자 금액의 자기자본 대비 비율, 자금 조달 방법, 완공·가동 일정과 예상 수요를 확인하세요."
        action = "수요 근거와 자금 여력이 확인되면 성장 요인으로, 지연·차입 확대 시 위험 요인으로 추적하세요."
    elif any(term in title_key for term in ["조회공시요구", "풍문또는보도", "해명공시", "답변(미확정)"]):
        meaning = "시장에 퍼진 보도나 소문에 대해 회사가 사실관계를 설명한 공시입니다."
        impact = "확정 여부와 구체적 조건이 확인되기 전에는 기대나 우려가 가격에 과도하게 반영될 수 있습니다."
        check = "회사가 확인한 사실, 아직 미확정인 항목, 재공시 예정일과 거래 조건을 확인하세요."
        action = "미확정 답변은 확정 사실로 보지 말고 재공시가 나올 때까지 별도 추적하세요."
    elif any(term in title_key for term in ["기업설명회", "IR개최"]):
        meaning = "회사가 투자자에게 사업 현황이나 실적을 설명할 일정을 알린 공시입니다."
        impact = "행사 안내만으로 실적 변화가 생긴 것은 아니며, 발표 자료에 새 전망이 포함될 때 영향이 생깁니다."
        check = "발표 자료에서 기존 전망과 달라진 매출·이익 목표, 투자 계획과 주주환원 내용을 확인하세요."
        action = "행사 일정은 관찰 대상으로 두고 실제 발표 내용이 나온 뒤 판단 근거를 갱신하세요."
    elif any(term in title_key for term in ["장래사업·경영계획", "경영계획(공정공시)"]):
        meaning = "회사가 앞으로 추진할 사업과 경영 목표를 공개하거나 정정한 공시입니다."
        impact = "매출·투자 목표가 구체적이면 성장 기대에 영향을 주지만 계획은 확정 실적과 다를 수 있습니다."
        check = "기존 계획에서 바뀐 항목, 정량 목표, 투자 금액, 실행 일정과 전제 조건을 확인하세요."
        action = "계획 발표만으로 실적을 확정하지 말고 이후 수주·투자 집행과 분기 실적 달성률을 추적하세요."
    elif any(term in title_key for term in ["기업가치제고계획", "밸류업계획"]):
        meaning = "수익성, 자본 효율과 주주환원을 높이기 위한 회사 계획 공시입니다."
        impact = "구체적인 목표와 실행 일정이 있으면 장기 기업가치에 긍정적일 수 있으나 선언만으로는 효과가 제한됩니다."
        check = "ROE·마진 목표, 배당·자사주 정책, 실행 일정과 정량 목표가 제시됐는지 확인하세요."
        action = "후속 실적과 집행 내역이 계획을 따라가는지 분기마다 추적하세요."
    elif "생산중단" in title_key or "영업정지" in title_key:
        meaning = "회사의 생산 또는 영업 일부가 중단되는 운영 차질 공시입니다."
        impact = "중단 기간과 대상 매출 비중이 크면 출하 감소와 비용 부담으로 단기 실적에 부정적일 수 있습니다."
        check = "중단 대상, 최근 매출 대비 비중, 중단 사유, 재개 예상일과 재고 대응 가능성을 확인하세요."
        action = "재개 일정과 실제 매출 영향이 확인될 때까지 실적 위험을 높여 추적하세요."
    elif "생산재개" in title_key or "영업재개" in title_key:
        meaning = "중단됐던 생산 또는 영업을 다시 시작한다는 공시입니다."
        impact = "정상 가동이 이어지면 매출 공백 우려가 줄지만 중단 기간의 손실과 회복 속도는 별도 확인이 필요합니다."
        check = "재개 일자, 정상 가동률, 밀린 주문 처리 계획과 중단 기간의 매출 손실을 확인하세요."
        action = "재개 발표 후 생산량과 매출이 실제로 회복되는지 다음 실적에서 확인하세요."
    elif any(term in title_key for term in ["소송", "횡령", "배임", "거래정지", "상장폐지"]):
        meaning = "법적 분쟁, 지배구조 또는 거래 위험과 관련된 공시입니다."
        impact = "청구 금액과 결과에 따라 비용, 현금흐름, 평판과 거래 가능성에 부담이 생길 수 있습니다."
        check = "청구 금액의 자기자본 대비 비중, 사건 단계, 회사 대응과 예상 일정을 확인하세요."
        action = "소송 결과가 확정되기 전에는 최대 손실 규모를 반영해 위험 한도를 점검하세요."
    elif any(term in title_key for term in ["단일판매", "공급계약", "수주"]) or taxonomy.get("disclosureCategory") == "operations-contract":
