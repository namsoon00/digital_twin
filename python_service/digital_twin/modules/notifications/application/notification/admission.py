...
from digital_twin.domain.data_freshness import evaluate_notification_data_freshness, sanitize_notification_context_for_freshness
from digital_twin.domain.message_types import INVESTMENT_INSIGHT
from digital_twin.domain.notification_rules import NotificationRuleConfig, apply_market_hours_rule, apply_similarity_rule, apply_state_cooldown_rule, attach_previous_profit_loss_context, evaluate_notification_rule, ontology_relation_delivery_diff, ontology_relation_delivery_metadata, notification_state_group_key, material_evidence_present
from digital_twin.domain.context_observation_notifications import typedb_context_observation_contract
from digital_twin.domain.notification_ai_context import relation_context_value
from digital_twin.domain.notifications import NotificationJob
from digital_twin.modules.notifications.domain.notification.lifecycle import age_minutes_since
from digital_twin.domain.sent_article_filter import news_story_changes_decision, news_story_is_decision_driver, news_story_impact_from_context


NON_BLOCKING_DELIVERY_REASONS = {
    "market_closed",
    "market_hours",
}
...
class NotificationAdmissionPolicy:
    ...
    def apply_result(
        ...
        if (
            ...
            and str(decision.suppression_reason or "") in NON_BLOCKING_DELIVERY_REASONS
            and not decision.state_suppressed
            and not decision.similarity_suppressed
        ):
            decision.should_send = True
            decision.delivery_state = "eligible"
        ...
        if not freshness.should_send:
            ...
            context["notificationFreshnessAdvisory"] = {
                ...
                "reason": str(freshness.reason or ""),
                "ageMinutes": freshness.age_minutes,
                "maxAgeMinutes": freshness.max_age_minutes,
                "staleSources": list(freshness.stale_sources or []),
            }
