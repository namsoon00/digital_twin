...
from digital_twin.domain.context_observation_notifications import context_observation_evidence_presentation, is_typedb_context_observation_notification, typedb_context_observation_contract
from digital_twin.domain.customer_investment_document import CustomerInvestmentDocument, CustomerInvestmentLink, CustomerInvestmentSection, customer_follow_up_condition_clause, customer_investment_document_quality, normalized_customer_investment_document
from digital_twin.domain.notification_ai import criterion_lines, notification_ai_prompt_context, relation_context_value
from digital_twin.domain.notification_ai_context import active_rule_items, relation_facts
from digital_twin.domain.notification_ai_context import is_watchlist_context
from digital_twin.domain.external_api_sources import external_api_source_line
from digital_twin.domain.notification_ai_gate_contracts import ACTION_LABELS, MESSAGE_START_BADGE, NotificationAIValidatedResponse
from digital_twin.domain.notification_icon_policy import notification_message_icon
from digital_twin.domain.notification_title_policy import investment_action_title, investment_notification_title
from digital_twin.domain.notification_explanation import build_notification_explanation_packet, normalize_notification_detail_level
from digital_twin.domain.notification_narrative import response_writer_provenance
from digital_twin.domain.notification_decision_policy import includes_portfolio_rebalance_policy
from digital_twin.domain.investment_ubiquitous_language import user_facing_investment_language
from digital_twin.domain.investment_decision_actionability import investment_decision_actionability, is_concrete_observable_condition
from digital_twin.domain.ontology_decision_state import ACTION_ENVELOPE_STATUS_LABELS, CHANGE_STATE_LABELS, DATA_STATE_LABELS, REVIEW_LEVEL_LABELS, VALIDATION_STATE_LABELS, review_level_for
from digital_twin.domain.notification_ai_gate_sources import source_detail_text
from digital_twin.domain.notification_reasoning_report import customer_alert_reason_lines, customer_data_state_and_missing_lines, customer_inferred_fact_lines
from digital_twin.domain.notification_ai_gate_text import _line_after_colon, _number, _raw_lines, _text, append_unique_text, customer_visible_ai_text, reference_date, user_friendly_ai_text
from digital_twin.domain.notification_ai_gate_validation import _driver_rows, action_label_for_target, delivery_level_from_context, watchlist_friendly_text
from digital_twin.domain.notification_start_badge import labeled_message_start_badge, notification_start_badge_required, strip_message_start_badge
from digital_twin.domain.notification_text_formatting import absolute_beginner_friendly_text, beginner_friendly_text
from digital_twin.domain.notification_ontology_sections import relation_axis_summary_lines
...
def prepend_execution_start_badge(rendered: str, context: Dict[str, object] = None) -> str:
    ...
    validated = values.get("notificationAiValidatedResponse")
    if not isinstance(validated, dict):
        validated = values.get("validatedDecisionResponse")
    strict_actionability = bool(
        values.get("notificationAiDecisionContractVersion")
        or values.get("_notificationAiPreparedDecisionCore")
    ...
    if (
        ...
        and str(
            investment_decision_actionability(values, validated).get("status") or ""
        ) == "review-only"
    ):
        # The body has already projected the failed executable opinion to a
        # safe review-only result. A stale pre-AI transition badge must not
        # reintroduce the rejected BUY/SELL instruction above that body.
...
def execution_headline(context: Dict[str, object], response: NotificationAIValidatedResponse) -> str:
    ...
    if str(response.action or "").upper() in {"BUY", "ADD"} and market_hours_message_rows(context):
        target = target_name_for_headline(
            context.get("displayTarget") or context.get("target") or context.get("title") or ""
        )
        label = "다음 장 소액 진입 검토" if is_watchlist_context(context) else "다음 장 추가매수 검토"
        return "🟡 " + (((target + " · ") if target else "") + label)
...
def compact_current_action_line(context: Dict[str, object], response: NotificationAIValidatedResponse) -> str:
    ...
    if is_typedb_context_observation_notification(context or {}):
        ...
        marker = "[판단 보류]" if str(response.action or "").upper() == "HOLD" else "[조건부 판단]"
    elif writer_kind == "ai":
        marker = "[AI]"
    else:
        marker = "[시스템 요약]"
...
def _compact_trend_from_facts(context: Dict[str, object]) -> str:
    ...
    for period in [5, 20, 60]:
        ...
        if average <= 0 and facts.get(distance_key) in (None, ""):
            continue
        distance = _number(facts.get(distance_key))
        if distance == 0 and current:
            distance = ((current / average) - 1) * 100 if average > 0 else 0
        direction = "높음" if distance >= 0 else "낮음"
        prefix = str(period) + "일선"
        if average > 0:
            prefix += " " + price_money(average, currency) + "보다 "
        else:
            prefix += "보다 "
        rows.append(prefix + str(abs(round(distance, 1))) + "% " + direction)
    return ", ".join(rows)
...
def compact_reason_is_internal(value: object) -> bool:
    ...
    return any(token in text for token in [
        ...
        "관계가 새로 감지", "관계 신호 관계", "다음 추론 세대", "추론 세대",
        "동일한 인과 경로", "watchlistontologysignal", "선택 가설", "검토 가설",
...
def full_decision_evidence_rows(
    ...
    *,
    ...
    if not counter:
        ...
        if canonical_rows and not any(
            row.startswith("선택 가설:") for row in canonical_rows
        ):
            return canonical_rows
...
def _decision_first_reason_rows(
    ...
    rows: List[str] = []
    interpretation = _investment_view_row(context, response)
    if interpretation and not compact_reason_is_internal(interpretation):
        append_unique_text(rows, interpretation, 420)
    macro_reason = compact_macro_constraint_reason(context)
    if macro_reason and not compact_reason_is_internal(macro_reason):
        append_unique_text(rows, macro_reason, 420)
    for item in evidence_rows:
        text = customer_visible_ai_text(item)
        if (
            text
            and not compact_reason_is_internal(text)
            and "사용자에게 표시할 성립값이 부족" not in text
            and not text.startswith("선택 가설:")
            and not text.startswith("확인 한계:")
        ):
            append_unique_text(rows, text, 420)
    return rows[:3]
...
def _customer_judgement_identity(
    ...
    from digital_twin.modules.notifications.contracts import notification_kind
...
def execution_telegram_message_decision_first(
    ...
    normalized_detail = normalize_notification_detail_level(detail_level)
    limits = {
        "concise": {"reason": 2, "counter": 1, "followUp": 3, "flow": 6},
        "standard": {"reason": 3, "counter": 2, "followUp": 4, "flow": 6},
    ...
    actionability = (
        investment_decision_actionability(context, response)
        if strict_actionability
        else {}
    )
    ...
    if review_only:
        ...
        factual_rows = [
            ...
            and not customer_visible_ai_text(item).startswith(("선택 가설:", "확인 한계:"))
        ]
        reason_rows = [review_reason]
        for item in factual_rows:
            append_unique_text(reason_rows, item, 420)
            if len(reason_rows) >= limits["reason"]:
                break
    counter_rows = [
        customer_visible_ai_text(item)
        for item in full_decision_evidence_rows(context, response, counter=True)
        if customer_visible_ai_text(item)
        and not compact_reason_is_internal(customer_visible_ai_text(item))
    ][:limits["counter"]]
    ...
    for item in compact_temporal_analysis_rows(context):
        append_unique_text(flow_rows, item, 360)
    flow_rows = flow_rows[:limits["flow"]]
    limitations = canonical_publication_data_warning_rows(context)
    for item in evidence_rows:
        text = customer_visible_ai_text(item)
        if text.startswith("확인 한계:") and not compact_reason_is_internal(text):
            append_unique_text(limitations, text, 360)
    for item in customer_data_note_rows(list(response.missing_data_impact)):
        append_unique_text(limitations, item, 360)
    unsupported = compact_provider_unsupported_line(context)
    if unsupported:
        append_unique_text(limitations, unsupported, 360)
    if review_only:
        gaps = set(actionability.get("gaps") or [])
        if "hypothesis-execution-qualification" in gaps:
            append_unique_text(
                limitations,
                "같은 조건의 독립된 사후 결과가 충분히 쌓이지 않았습니다.",
                360,
            )
        if "supported-causal-path" in gaps:
            append_unique_text(
                limitations,
                "현재 신호가 수급·실적 또는 위험 변화로 이어지는 경로가 확인되지 않았습니다.",
                360,
            )

    action_line = compact_current_action_line(context, response)
    market_rows = market_hours_message_rows(context)
    if review_only:
        action_line = (
        ...
        action_line = (
            "지금은 장이 닫혀 있어 주문하지 않습니다. 다음 거래가 시작되면 표시 가격과 "
    ...
    if not transition_state:
        transition_state = decision_transition_from_context(context)
    previous_action = str(transition_state.get("previousAction") or "").strip().upper()
    current_action = str(
        transition_state.get("currentAction") or response.action or ""
    ).strip().upper()
