...
from digital_twin.domain.investment_reasoning import FactDelta, rule_evaluation_records_from_projection_results
from digital_twin.domain.market_observation_reasoning import market_observation_completion_scope
from digital_twin.domain.ontology_projection_input import compact_monitor_state_for_ontology
from digital_twin.domain.world_partitioned_reasoning import attach_shared_premise_evidence
from digital_twin.domain.ontology_store_routing import ontology_store_route
...
class V2ReasoningEngine:
    ...
    def execute(
        ...
        if reasoning_case is not None and verified_accounts:
            ...
            ready_review_symbols = {
                str(getattr(event, "symbol", "") or "").strip().upper()
                for event in ready_events
                if str(getattr(event, "symbol", "") or "").strip()
                and typedb_review_observation_contract(
                    getattr(event, "metadata", {}) or {}
                )
            }
            review_observation_symbols = sorted(
                symbol
                for symbol, events in events_by_symbol.items()
                if symbol in ready_review_symbols
                and events
                and all(
            ...
            if review_observation_symbols:
                reasoning_case = self.reasoning_orchestrator.review_observation_validated(
                    reasoning_case.case_id,
                    "TypeDB가 위험·제약 관계를 검증했지만 매매 행동 생성 권한은 부여하지 않았습니다.",
                    review_observation_symbols,
                )
