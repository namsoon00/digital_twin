...
from digital_twin.modules.notifications.contracts import compact_number, price_money
from digital_twin.modules.news_intelligence.contracts import company_prompt_context, company_valuation_context
from digital_twin.modules.news_intelligence.contracts import research_evidence_from_external_signals, research_evidence_from_facts
from digital_twin.modules.market_data.contracts import investor_flow_contract, investor_flow_observation, investor_flow_values_reliable
from digital_twin.modules.market_data.contracts import macro_context_facts
...
from digital_twin.modules.news_intelligence.contracts import news_state_payload as _contract_news_domain_news_state_payload
from digital_twin.modules.news_intelligence.contracts import relation_scope_is_investable as _contract_news_domain_relation_scope_is_investable
...
from digital_twin.modules.model_registry.contracts import BTC_SENSITIVE_SYMBOLS
from digital_twin.modules.portfolio.contracts import external_valuation_rows, position_runtime_valuation_rows, quality_checked_valuation_row
...
def _valuation_row_payload(position: Position, row: Dict[str, object]) -> Dict[str, object]:
    ...
    payload = {
        ...
        "valuationConfidence": str(row.get("valuationConfidence") or "").strip(),
        "epsScenario": dict(row.get("epsScenario") or {}) if isinstance(row.get("epsScenario"), dict) else {},
        "multipleBand": dict(row.get("multipleBand") or {}) if isinstance(row.get("multipleBand"), dict) else {},
        "formulaTrace": dict(row.get("formulaTrace") or {}) if isinstance(row.get("formulaTrace"), dict) else {},
        "inputObservations": [dict(item) for item in row.get("inputObservations") or [] if isinstance(item, dict)],
        "familyEvidence": [dict(item) for item in row.get("familyEvidence") or [] if isinstance(item, dict)],
        "modelExclusionReasons": [str(item) for item in row.get("modelExclusionReasons") or [] if str(item or "").strip()],
        ...
        "valuationReproducibilityGaps": [str(item) for item in row.get("valuationReproducibilityGaps") or [] if str(item or "").strip()],
        "expectedEPSLow": number(row.get("expectedEPSLow")),
        "expectedEPSHigh": number(row.get("expectedEPSHigh")),
        "targetPERLow": number(row.get("targetPERLow")),
        "targetPERHigh": number(row.get("targetPERHigh")),
...
def _valuation_facts(position: Position, external_signals: Dict[str, object], settings: Optional[Dict[str, object]]) -> Dict[str, object]:
    ...
    if not rows:
        return {
            ...
            "valuationConfidence": "",
            "valuationEpsScenario": {},
            "valuationMultipleBand": {},
            "valuationFormulaTrace": {},
            "valuationInputObservations": [],
            "valuationFamilyEvidence": [],
            "valuationModelExclusionReasons": [],
            "valuationBundleId": "",
            "valuationAssessmentId": "",
            "valuationAuditFingerprint": "",
            "valuationMaterialFingerprint": "",
            "valuationReproducibilityState": "unavailable",
            "valuationReproducibilityGaps": ["valuation-input-bundle-missing"],
        }
    ...
    return {
        ...
        "valuationCurrentPER": number(primary.get("peRatio")),
        "valuationPBR": number(primary.get("pbr")),
        ...
        "valuationModelVersion": primary.get("modelVersion"),
        "valuationConfidence": primary.get("valuationConfidence"),
        "valuationEpsScenario": primary.get("epsScenario") if isinstance(primary.get("epsScenario"), dict) else {},
        "valuationMultipleBand": primary.get("multipleBand") if isinstance(primary.get("multipleBand"), dict) else {},
        "valuationFormulaTrace": primary.get("formulaTrace") if isinstance(primary.get("formulaTrace"), dict) else {},
        ...
        "valuationBundleId": primary.get("valuationBundleId"),
        "valuationAssessmentId": primary.get("valuationAssessmentId"),
        "valuationAuditFingerprint": primary.get("valuationAuditFingerprint"),
        "valuationMaterialFingerprint": primary.get("valuationMaterialFingerprint"),
        "valuationReproducibilityState": primary.get("valuationReproducibilityState"),
        "valuationReproducibilityGaps": primary.get("valuationReproducibilityGaps") if isinstance(primary.get("valuationReproducibilityGaps"), list) else [],
        ...
        "valuationTargetPERHigh": number(primary.get("targetPERHigh")),
    }
...
def _evidence_relevance_state(item: Dict[str, object]) -> str:
    payload = _evidence_payload(item)
    return _contract_news_domain_news_state_payload({**payload, **item})["relevanceState"]


def _evidence_materiality_state(item: Dict[str, object]) -> str:
    payload = _evidence_payload(item)
    return _contract_news_domain_news_state_payload({**payload, **item})["materialityState"]


def _evidence_source_trust_state(item: Dict[str, object]) -> str:
    payload = _evidence_payload(item)
    return _contract_news_domain_news_state_payload({**payload, **item})["sourceTrustState"]
...
def research_evidence_facts(evidence_items: Iterable[Dict[str, object]]) -> Dict[str, object]:
    news_items = [
        ...
        and _contract_news_domain_relation_scope_is_investable(_evidence_scope(item))
