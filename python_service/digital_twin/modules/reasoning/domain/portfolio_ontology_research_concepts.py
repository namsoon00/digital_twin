...
from digital_twin.modules.news_intelligence.contracts import disclosure_reasoning_eligibility
from digital_twin.modules.reasoning.domain.ontology_contracts import OntologyEvidence, PortfolioOntology
...
def research_evidence_state(item: object, raw_payload: Dict[str, object]) -> Dict[str, object]:
    ...
    news_states = _contract_news_domain_news_state_payload(raw_payload)
    return {
        "investmentJudgmentEligible": bool(eligible),
...
def add_research_evidence_concepts(
    ...
    for item in evidence_by_id.values():
        ...
        if item.kind == "news" and not _contract_news_domain_relation_scope_is_investable(relation_scope):
            continue
