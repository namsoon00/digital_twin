...
from digital_twin.domain.hypothesis_calibration import hypothesis_calibration_snapshot_from_abox_rows
from digital_twin.domain.ontology_contracts import PortfolioOntology
from digital_twin.domain.ontology_scopes import SCOPED_ABOX_MANIFEST_VERSION
from digital_twin.infrastructure.graph_store_payloads import condition_relation_filter_values
...
def has_box_rows(_store: GraphReadsRowsStore, box: str, world_id: str='') -> bool:
    ...
    if clean_box == "ABox":
        ...
        if (
            ...
            return True
        query = (
            "match " + _store.active_abox_members_clause([("$n", "boxProbe")], world_id) + " "
...
def read_entity_rows(_store: GraphReadsRowsStore, boxes: Iterable[str]=None, limit: int=0, world_id: str='', snapshot_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
    ...
    for box in normalized:
        ...
        query = (
            ...
            + _bindings.typeql_limit_clause(safe_limit)
        )
        rows.extend(_store.entity_rows_from_typeql(_store.read_rows(
            query,
...
def read_active_hypothesis_calibration_rows(_store: GraphReadsRowsStore, symbols: Iterable[str]=None, limit: int=40, world_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
    ...
    rows = _store.entity_rows_from_typeql(_store.read_rows(
        query,
...
def hypothesis_calibration_snapshot(_store: GraphReadsRowsStore, symbols: Iterable[str]=None, limit: int=40, world_id: str='', source_abox_snapshot_id: str='', generation_aligned: bool=False) -> Dict[str, object]:
...
def hypothesis_calibration_snapshot_for_native_result(_store: GraphReadsRowsStore, matched_graph: PortfolioOntology, symbols: Iterable[str], source_abox_snapshot_id: str, generation_aligned: bool, scoped_active_abox: bool, limit: int=40, world_id: str='') -> Dict[str, object]:
    ...
    return hypothesis_calibration_snapshot_from_abox_rows(
        [
            row for row in _store.rows_for_entities(matched_graph)
...
def read_entity_rows_by_ids(_store: GraphReadsRowsStore, ids: Iterable[str], boxes: Iterable[str]=None, world_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
...
def read_abox_entity_rows_by_storage_ids(_store: GraphReadsRowsStore, storage_ids: Iterable[str], world_id: str='') -> List[Dict[str, object]]:
    ...
    for offset in range(0, len(clean_storage_ids), NATIVE_RULE_EVIDENCE_READ_INDEX_BATCH_SIZE):
        ...
        rows.extend(_store.entity_rows_from_typeql(
            _store.read_rows(
                query,
                ["id", "label", "kind", "updatedAt", "json"],
...
def read_abox_relation_rows_by_storage_ids(_store: GraphReadsRowsStore, storage_ids: Iterable[str], relation_types: Iterable[str]=None, world_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
    ...
    for offset in range(0, len(clean_storage_ids), NATIVE_RULE_EVIDENCE_READ_INDEX_BATCH_SIZE):
        ...
        query = (
            ...
            + typedb_value_match("$r", "ontology-storage-id", batch, "==", "relationStorageIdFilter")
...
def read_relation_rows_by_source_ids(_store: GraphReadsRowsStore, source_ids: Iterable[str], boxes: Iterable[str]=None, relation_types: Iterable[str]=None, include_incoming: bool=True, world_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
    ...
    for box in _bindings.normalized_boxes(boxes):
        ...
        endpoint_scope = ""
        if box == "ABox":
            active_scope = _store.active_abox_members_clause([
                ("$source", "sourceById"),
                ("$target", "targetById"),
                ("$r", "relationById"),
            ], world_id) + " "
...
def active_abox_relation_types_by_symbol(_store: GraphReadsRowsStore, symbols: Iterable[str]=None, timeout_seconds: float=None, world_id: str='', active_abox_metadata: Dict[str, object]=None) -> Dict[str, object]:
...
def active_abox_rule_context(_store: GraphReadsRowsStore, symbols: Iterable[str], world_id: str='') -> Dict[str, object]:
...
def read_relation_rows(_store: GraphReadsRowsStore, boxes: Iterable[str]=None, limit: int=0, world_id: str='', snapshot_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
    rows: List[Dict[str, object]] = []
    safe_limit = int(limit or 0)
    normalized = _bindings.normalized_boxes(boxes)
    static_generations: Dict[str, str] = {}
    ...
    for box in normalized:
        ...
        rows.extend(_store.relation_rows_from_typeql(_store.read_rows(
            query,
...
def read_inferencebox_entity_rows(_store: GraphReadsRowsStore, generation_id: str='', symbols: Iterable[str]=None, limit: int=0, world_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
    safe_limit = int(limit or 0)
...
def read_inferencebox_relation_rows(_store: GraphReadsRowsStore, generation_id: str='', symbols: Iterable[str]=None, limit: int=0, world_id: str='', *, _bindings: GraphReadsRowsRuntime) -> List[Dict[str, object]]:
    safe_limit = int(limit or 0)
    clean_symbols = sorted(set(str(item or "").upper().strip() for item in (symbols or []) if str(item or "").strip()))
    query = (
        "match "
    ...
    query += _bindings.typeql_limit_clause(safe_limit)
    rows = _store.relation_rows_from_typeql(_store.read_rows(
        query,
        ["id", "sourceId", "sourceLabel", "targetId", "targetLabel", "type", "updatedAt", "json", "weight"],
    ), "InferenceBox")
    rows = sorted(rows, key=lambda item: (str(item.get("updatedAt") or ""), str(item.get("source") or ""), str(item.get("target") or "")), reverse=True)
...
def entity_row_from_typeql(_store: GraphReadsRowsStore, row: Dict[str, object], box: str, *, _bindings: GraphReadsRowsRuntime) -> Dict[str, object]:
    ...
    payload = {
        **merged,
        ...
        "derivationTboxClasses": _bindings.list_of_strings(derivation.get("tbox_classes") or merged.get("derivationTboxClasses")),
        ...
        "derivationAllowedActions": _bindings.list_of_strings(derivation.get("allowed_actions") or derivation.get("allowedActions") or merged.get("derivationAllowedActions")),
        "derivationBlockedActions": _bindings.list_of_strings(derivation.get("blocked_actions") or derivation.get("blockedActions") or merged.get("derivationBlockedActions")),
        ...
        "derivationBlockedActionLabels": _bindings.list_of_strings(derivation.get("blocked_action_labels") or derivation.get("blockedActionLabels") or merged.get("derivationBlockedActionLabels")),
        "derivationStrengthenConditions": _bindings.list_of_strings(derivation.get("strengthen_conditions") or derivation.get("strengthenConditions") or merged.get("derivationStrengthenConditions")),
        "derivationWeakenConditions": _bindings.list_of_strings(derivation.get("weaken_conditions") or derivation.get("weakenConditions") or merged.get("derivationWeakenConditions")),
        "derivationNextChecks": _bindings.list_of_strings(derivation.get("next_checks") or derivation.get("nextChecks") or merged.get("derivationNextChecks")),
...
def relation_row_from_typeql(_store: GraphReadsRowsStore, row: Dict[str, object], box: str, *, _bindings: GraphReadsRowsRuntime) -> Dict[str, object]:
    props = json_object(row.get("json"))
    merged = _bindings.merge_flat_properties({
