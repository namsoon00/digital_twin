...
class NotificationReverseReasoningTests(unittest.TestCase):
    ...
    def test_web_renders_reasoning_in_execution_order_without_slicing_rules(self):
        ...
        self.assertIn('data-notification-detail-tab', source)
        self.assertIn('["reasoning", "규칙 증명"]', source)
        self.assertIn('["ai-review", "AI 실행·검증"]', source)
        self.assertIn("loadNotificationJobDetailSection", source)
        self.assertIn("renderNotificationRuleProofSection", source)
    ...
    def test_detail_endpoint_exposes_trace_without_bloating_list_payload(self):
        ...
        with mock.patch("digital_twin.infrastructure.web_server.notification_queue_store", return_value=Queue()), \
            ...
            reasoning = notification_job_detail_payload(job.job_id, section="reasoning")
            ai_review = notification_job_detail_payload(job.job_id, section="ai-review", include_sensitive=False)
            delivery = notification_job_detail_payload(job.job_id, section="delivery")
        ...
        self.assertEqual("ai-review", ai_review["section"])
        self.assertNotIn("prompt", ai_review["aiExecution"])
        self.assertNotIn("must-not-leak", json.dumps(ai_review, ensure_ascii=False))
        self.assertEqual("delivery", delivery["section"])
        self.assertEqual(3, delivery["pipeline"]["stageCount"])
