...
class OntologyInferenceContextTests(unittest.TestCase):
    ...
    def test_position_signal_facts_use_kis_domestic_fundamentals_for_kr_stock(self):
        ...
        self.assertEqual("external", facts["valuationSourceType"])
        self.assertEqual("earnings-context", facts["valuationMethod"])
        self.assertEqual(0, facts["valuationFairValue"])
        self.assertEqual(10000, facts["valuationExpectedEPS"])
        self.assertEqual(0, facts["valuationTargetPER"])
        self.assertEqual("최근 실적 EPS 참고", facts["valuationFormula"])
        self.assertEqual("missing", facts["valuationPerStatus"])
        self.assertIn("EPS 또는 PER가 없어", facts["valuationPerReason"])
        self.assertEqual("적정가 입력 또는 외부 PER/EPS", facts["valuationPreferredMetric"])
        self.assertEqual(0, facts["valuationFairValueLow"])
        self.assertEqual(0, facts["valuationFairValueHigh"])
        self.assertEqual("", facts["valuationModelVersion"])
        self.assertEqual("", facts["valuationConfidence"])
        self.assertIn("targetPER", facts["valuationMissingInputs"])
