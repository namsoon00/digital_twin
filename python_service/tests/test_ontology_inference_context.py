...
class OntologyInferenceContextTests(unittest.TestCase):
    ...
    def test_position_signal_facts_convert_underlying_kis_fundamentals_for_adr(self):
        ...
        self.assertAlmostEqual(0.7143, facts["valuationExpectedEPS"], places=4)
        self.assertTrue(facts["valuationFormulaTrace"]["adrConversionApplied"])
        self.assertEqual(0.1, facts["valuationFormulaTrace"]["adrRatio"])
        self.assertEqual(1400, facts["valuationFormulaTrace"]["fxRate"])
        self.assertIn("연간 EPS 시나리오", facts["valuationFormula"])
        self.assertIn("메모리 업황 또는 실적 사이클 지표", facts["valuationMissingInputs"])
        self.assertEqual("provisional", facts["valuationPerStatus"])
        self.assertIn("PER 표본이 부족", facts["valuationPerReason"])
        self.assertIn("KIS/yfinance 예상 EPS", facts["valuationFundamentalDataSourcePriority"])
