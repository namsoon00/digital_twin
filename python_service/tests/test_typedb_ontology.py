...
class TypeDBOntologyRepositoryTests(unittest.TestCase):
    ...
    def test_typedb_schema_sync_bootstraps_from_empty_text_when_schema_inspection_fails(self):
        repository = TypeDBOntologyGraphRepository(
            "typedb-schema-read-failure.test:1729",
            database="schema_read_failure_test",
        )
        repository.invalidate_process_base_schema_readiness()
        imported = (object, object, object, object, SimpleNamespace(SCHEMA="schema"))
        ...
        with patch.object(
            repository,
            "typedb_schema_text",
            side_effect=RuntimeError("fresh candidate schema unavailable"),
        ), patch.object(
            repository,
    ...
    def test_typedb_base_schema_batch_plan_orders_parents_and_resumes_partial_schema(self):
        ...
        self.assertLessEqual(
            max(int(item.get("definitionCount") or 0) for item in full_plan),
            64,
        )
    ...
    def test_stock_targeted_execution_index_retains_portfolio_aggregate_facts(self):
        ...
        combined_plan = typedb_native_indexed_evidence_match_query(
        ...
        self.assertEqual("ok", combined_plan["status"])
        self.assertTrue(combined_plan["anyConditionsVerified"])
        self.assertEqual(
            "typedb-manifest-evidence-index-any-combined",
            combined_plan["queryMode"],
        )
        self.assertIn(
            index["sourceStorageIdsBySourceId"]["portfolio:default"],
            combined_plan["query"],
        ...
        self.assertNotIn(
            index["sourceStorageIdsBySourceId"]["stock:MSTR"],
            combined_plan["query"],
        )
        self.assertEqual(2, combined_plan["storageIdentityCount"])
        self.assertNotIn("reduce $anyConditionCount = count", combined_plan["query"])
        self.assertNotIn("$anyConditionToken", combined_plan["query"])
    ...
    def test_typedb_native_rule_execution_plan_prunes_impossible_rules_before_function_calls(self):
        ...
        self.assertNotIn("graph.holding.trend_transition.risk.v1", selected_ids)
    ...
    def test_manifest_relation_evidence_prunes_a_proven_news_filter_mismatch(self):
        rule = governed_catalog_rule("graph.news.direct_material_support.v1")
        relation_evidence = {
            "005930": [
                {
    ...
    def test_typedb_native_rule_preflight_prunes_ineligible_model_signal(self):
        ...
        graph.entities.extend([
            ...
            OntologyEntity("signal:005930", "가격 회복 모델 신호", "statistical-model-hypothesis-evidence", {
                "ontologyBox": "ABox",
                "signalType": "price-recovery-support",
                "hypothesisContractId": "graph.watchlist.temporal.recovery_entry.v1",
    ...
    def test_typedb_native_rule_preflight_keeps_incoming_relation_rules_when_endpoint_scan_is_deferred(self):
        rule = governed_catalog_rule("graph.news.ai_direct_risk.v1")
        graph = PortfolioOntology(
    ...
    def test_typedb_native_rule_query_uses_stored_semantic_subtypes(self):
        rule = next(
            item
            for item in governed_graph_inference_rules()
    ...
    def test_model_rule_migration_activates_only_automatically_disabled_waiting_rule(self):
        bootstrap = rulebox_rules_to_payload(default_graph_inference_rules())
        expected = next(
            deepcopy(item) for item in bootstrap
            if item["rule_id"] == "graph.loss_guard.breakdown.v1"
        )
        waiting = deepcopy(expected)
        waiting["version"] = "legacy-raw-market-rule-v1"
        waiting["enabled"] = False
        waiting["knowledge_basis"]["migrationDisposition"] = "awaiting-governed-model-scorer"

        migration = migrate_typedb_rule_catalog([waiting], bootstrap)
        migrated = {item["rule_id"]: item for item in migration["rules"]}

        self.assertTrue(migrated[waiting["rule_id"]]["enabled"])
        self.assertIn(waiting["rule_id"], migration["modelSignalUpdatedRuleIds"])

    def test_model_rule_migration_preserves_operator_disabled_production_rule(self):
        bootstrap = rulebox_rules_to_payload(default_graph_inference_rules())
        stored = next(
            deepcopy(item) for item in bootstrap
            if item["rule_id"] == "graph.loss_guard.breakdown.v1"
        )
        stored["version"] = "statistical-rule-model-signal-production-v2"
        stored["enabled"] = False

        migration = migrate_typedb_rule_catalog([stored], bootstrap)
        migrated = {item["rule_id"]: item for item in migration["rules"]}

        self.assertFalse(migrated[stored["rule_id"]]["enabled"])
        self.assertIn(stored["rule_id"], migration["modelSignalUpdatedRuleIds"])
        ...
        class FakeRepository:
            ...
            def __init__(self):
                ...
                self.pending = {
                    "status": "pending",
                    "activationStatus": "staged-native-inference",
                    "candidateAboxSnapshotId": "abox-manifest:staged",
                    "previousAboxSnapshotId": "abox-manifest:previous",
                    "targetSymbols": ["AAPL"],
                }
            ...
            def run_rulebox(self, payload):
                ...
                if payload["symbols"] != ["AAPL"]:
                    raise AssertionError("staged candidate must retain its original target symbol")
                return {"status": "ok"}
            ...
            def inferencebox_snapshot(self, *_args, **_kwargs):
                ...
                return {
                    ...
                    "nativeTypeDbReasoningUsed": True,
                    "generationAligned": True,
                    "sourceAboxSnapshotId": "abox-manifest:staged",
                    "targetSymbols": ["AAPL"],
                }
            ...
            def finalize_abox_generation(self, active_snapshot_id, previous_snapshot_id, **_kwargs):
                ...
                if active_snapshot_id != "abox-manifest:staged" or previous_snapshot_id != "abox-manifest:previous":
                    raise AssertionError("staged candidate finalization used the wrong generation")
                return {"status": "ok"}
    ...
    def test_service_manager_restarts_only_candidate_between_seed_attempts(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            results = [
                SimpleNamespace(returncode=1, stdout="", stderr="schema timeout"),
            ...
            with patch.object(service_manager, "typedb_rotation_lock_path", return_value=Path(temp) / "typedb-rotation.lock"), \
                    ...
                    patch.object(
                        service_manager,
                        "restart_typedb_stage_for_seed_retry",
                        return_value=True,
                    ) as restart_candidate, \
                    patch.object(service_manager.time, "sleep") as sleep:
                self.assertTrue(service_manager.ensure_typedb_seeded(spec))
