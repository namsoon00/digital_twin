...
function checkWorkflowConsoleContract() {
  ...
  assertOk(
    webServer.indexOf("def request_symbol_universe_refresh") >= 0 &&
      ...
      code.indexOf("function renderSymbolUniverseRefreshStageRail") >= 0 &&
      code.indexOf("function renderSymbolUniverseNavTask") >= 0 &&
      code.indexOf("function renderSymbolUniverseRefreshHistory") >= 0 &&
      code.indexOf('data-action=\"retry-symbol-refresh\"') >= 0 &&
      code.indexOf('event.target.closest(\'[data-action="refresh-symbol-universe"]\')') >= 0 &&
    ...
    "전체 종목 갱신의 단계형 진행, 상태 폴백, 다중 버튼, 실패 복구, 전역 작업 표시 또는 목록 맥락 보존 계약이 없습니다."
  );
  ...
  assertOk(
    code.indexOf("function notePageScrollActivity") >= 0 &&
      code.indexOf("pageScrollRecentlyActive()") >= 0 &&
      code.indexOf("restoreRenderedPageScrollPositionAfterLayout(renderedScrollPosition)") >= 0 &&
      code.indexOf("appNavScrollDistance >= 44") >= 0 &&
      code.indexOf('(appNavHidden ? " is-hidden" : "")') >= 0 &&
      code.indexOf("var snackbar = state.snackbar || {}") >= 0 &&
      code.indexOf("var visible = active || unacknowledged") >= 0 &&
      styles.indexOf("Mobile native vertical scrolling") >= 0 &&
      styles.indexOf("backdrop-filter: none !important") >= 0 &&
      styles.indexOf("touch-action: pan-x pan-y pinch-zoom") >= 0,
  ...
  assertOk(
    indexHtml.indexOf('id="app-request-progress"') >= 0 &&
      code.indexOf("function beginNetworkActivity") >= 0 &&
      code.indexOf("function endNetworkActivity") >= 0 &&
      code.indexOf("function bindNetworkActivityControls") >= 0 &&
      code.indexOf("function decorateRenderedBusyControls") >= 0 &&
      code.indexOf('control.setAttribute("aria-busy", "true")') >= 0 &&
      styles.indexOf("Shared network request feedback") >= 0 &&
      styles.indexOf(".app-request-progress.is-active") >= 0 &&
      styles.indexOf(".button-activity-spinner") >= 0 &&
      styles.indexOf("@keyframes oa-request-progress") >= 0,
    "공통 API 진행 바와 버튼별 조회·저장 진행 상태 계약이 없습니다."
  );
  assertOk(
...
function checkFrontendAdminRender() {
  ...
  return Promise.all([
    ...
    renderForSearch("?tab=experiments&lab=promotion"),
    renderForSearch("?tab=experiments&lab=proposals")
  ]).then(function (pages) {
  ...
  assertOk(accountResultsHtml.indexOf("page-mode-switch") >= 0 && accountResultsHtml.indexOf('data-page-mode="results"') >= 0 && accountResultsHtml.indexOf('data-page-mode="settings"') >= 0, "계정 탭에 결과/설정 전환이 없습니다.");
    ...
    const accountChangesSource = code.match(/function accountPayloadChanges\(payload, baseline\) \{[\s\S]*?\n  \}/);
    assertOk(accountChangesSource, "Missing account partial-update contract");
    const accountChangesSandbox = {};
    vm.runInNewContext(accountChangesSource[0], accountChangesSandbox);
    const originalAccount = { id: "module-test", label: "Test", watchlistSymbols: "AAPL", quietHoursEnd: "05:00", enabled: true };
    const changedAccount = Object.assign({}, originalAccount, { quietHoursEnd: "08:00" });
    assertOk(JSON.stringify(accountChangesSandbox.accountPayloadChanges(changedAccount, originalAccount)) === JSON.stringify({ id: "module-test", quietHoursEnd: "08:00" }), "Account preference edits must not resend a stale watchlist");
    assertOk(JSON.stringify(accountChangesSandbox.accountPayloadChanges(originalAccount, originalAccount)) === JSON.stringify({ id: "module-test" }), "Unchanged account fields must not be rewritten");
    assertOk(accountChangesSandbox.accountPayloadChanges(changedAccount, null) === changedAccount, "New accounts need the full creation payload");
    assertOk(accountChangesSandbox.accountPayloadChanges(Object.assign({}, originalAccount, { clientSecret: "test-only-input" }), originalAccount).clientSecret === "test-only-input", "Explicit credential edits must be preserved");
    ...
    assertOk(
      notificationHtml.indexOf("notification-decision-detail") >= 0 &&
        ...
        code.indexOf("notificationDetailDisclosureOpen: {}") >= 0 &&
        code.indexOf('details.getAttribute("data-notification-detail-disclosure-key")') >= 0 &&
        code.indexOf("state.notificationDetailDisclosureOpen[disclosureKey] = Boolean(details.open)") >= 0 &&
        code.indexOf("detailDisclosurePrefix + \"reasoning\"") >= 0 &&
        code.indexOf("reasoningDisclosurePrefix + \"6\"") >= 0 &&
        code.indexOf("(disclosureOpen ? ' open' : '')") >= 0 &&
    ...
    assertOk(modelingChartHtml.indexOf("investment-tab-workspace-charts") >= 0 && modelingChartHtml.indexOf("investment-chart-control-panel") >= 0 && modelingChartHtml.indexOf("investment-chart-panel") >= 0 && modelingChartHtml.indexOf("investment-chart-periods") >= 0, "통합 차트 섹션이 기준선 패널과 기간 컨트롤로 렌더링되지 않습니다.");
    ...
    assertOk(modelingTraceHtml.indexOf("strategy-review-status-panel") >= 0 && modelingTraceHtml.indexOf("검증 상태판") >= 0 && modelingTraceHtml.indexOf("strategy-trace-overview-panel") >= 0, "검증·리뷰 섹션이 상단 상태판과 상세 진입 카드로 분리되지 않았습니다.");
    assertOk(modelingProposalsHtml.indexOf("strategy-proposal-console-panel") >= 0 && modelingProposalsHtml.indexOf("strategy-proposal-layout") >= 0, "투자 판단 전략제안 섹션이 공용 승인 큐 콘솔로 렌더링되지 않습니다.");
    assertOk(code.indexOf("renderStrategyProposalSectionTabs") >= 0 && code.indexOf("data-strategy-proposal-section") >= 0 && styles.indexOf(".strategy-proposal-section-tabs") >= 0, "전략제안 상세가 요약/조건/검증/성과/이력 내부 섹션으로 분리되지 않았습니다.");
    assertOk(styles.indexOf(".strategy-proposal-performance-form") >= 0 && styles.indexOf("grid-template-columns: repeat(3, minmax(0, 1fr));") >= 0 && styles.indexOf(".strategy-proposal-performance-form .setting-field.wide") >= 0, "전략제안 성과 입력 폼이 PC에서 과도한 6컬럼 구조를 벗어나지 못했습니다.");
